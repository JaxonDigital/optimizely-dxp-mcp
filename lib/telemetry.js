...
class Telemetry {
    ...
    sendSessionStart() {
        ...
        const event = {
            ...
            event: {
                ...
                platform: os.platform(),
                arch: os.arch(),
                // Include AI client and location in session start (nested structure as per OCA requirements)
                ai_client: this.metrics.environment.aiClient,
    ...
    trackToolCall(toolName, duration, args = {}, success = true, error = null) {
        ...
        try {
            ...
            const event = {
                ...
                // Include AI client info in tool calls (standardized format)
    ...
    async sendToAnalyticsDashboard(events) {
        ...
        try {
            ...
            const transformedEvents = events.map(event => {
                ...
                if (event.type === 'tool_invocation' || event.type === 'tool_error' || event.tool_name) {
                    return {
                        ...
                        // Include AI client data in analytics format (standardized)
                        ...
                        location: {
                            timezone: this.metrics.environment.location?.timezone,
                            region: this.metrics.environment.location?.region,
                            countryCode: this.metrics.environment.location?.countryCode,
                            locale: this.metrics.environment.location?.locale,
                            utcOffset: this.metrics.environment.location?.utcOffset
                        }
                    };
                }
                ...
                if (event.type === 'session_start' || event.type === 'session_end') {
                    // Handle nested event structure for session events
                    const aiClientData = event.event?.ai_client || event.ai_client || this.metrics.environment.aiClient;
                    ...
                    return {
                        ...
                        // Properly extract AI client fields from the nested structure
                        ai_client: aiClientData?.name || 'unknown',
                        ai_client_version: aiClientData?.version || '1.0.0',
                        location: event.event?.location || event.location || this.metrics.environment.location
                    };
                }
            ...
            const data = JSON.stringify(transformedEvents);
    ...
    getSessionSummary() {
        ...
        return {
            ...
            summary: {
                ...
                // Include AI client and location in session summary
