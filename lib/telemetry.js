...
class Telemetry {
    ...
    /**
     * Simple, direct tool invocation tracking (like log-analyzer MCP)
     * This bypasses all complex transformation logic
     */
    trackToolDirect(toolName, duration, success = true) {
        if (!this.enabled || !this.analyticsEndpoint) return;

        // Create event exactly like working log-analyzer MCP
        const event = {
            timestamp: new Date().toISOString(),
            sessionId: this.sessionId,
            type: 'tool_invocation',
            tool_name: toolName,  // DIRECT assignment - no transformation
        ...
        // Send COMPLETELY directly - bypass ALL transformation
    ...
    /**
     * Send events directly to analytics with NO transformation
     * Bypasses all the complex logic that corrupts tool_name
     */
    async sendDirectToAnalytics(events) {
        ...
        try {
            ...
            if (process.env.DEBUG) {
                console.error('[TELEMETRY] Direct send failed:', error.message);
            }
        }
    }
    ...
    // Legacy sendEvents method removed - using analytics dashboard only
