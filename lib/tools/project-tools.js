...
class ProjectTools {
    ...
    static getConfiguredProjects() {
        ...
        // Check for OPTIMIZELY_PROJECTS JSON array
        if (process.env.OPTIMIZELY_PROJECTS) {
            try {
                const projectList = JSON.parse(process.env.OPTIMIZELY_PROJECTS);
                if (Array.isArray(projectList)) {
                    projectList.forEach(project => {
                        projects.push({
                            ...
                            environments: project.environments || ['Integration', 'Preproduction', 'Production'],
                            isDefault: project.isDefault || false
                        });
                    });
                }
            } catch (e) {
                console.error('Failed to parse OPTIMIZELY_PROJECTS:', e);
            }
        }
        
        // Add default project from individual env vars if exists
        if (process.env.OPTIMIZELY_PROJECT_ID) {
            const defaultProject = {
                name: process.env.OPTIMIZELY_PROJECT_NAME || 'Default Project',
                id: process.env.OPTIMIZELY_PROJECT_ID,
                apiKey: process.env.OPTIMIZELY_API_KEY,
                apiSecret: process.env.OPTIMIZELY_API_SECRET,
                environments: process.env.OPTIMIZELY_ALLOWED_ENVIRONMENTS 
                    ? process.env.OPTIMIZELY_ALLOWED_ENVIRONMENTS.split(',').map(e => e.trim())
                    : ['Integration', 'Preproduction', 'Production'],
                isDefault: true
            };
            
            // Only add if not already in list
            if (!projects.find(p => p.id === defaultProject.id)) {
                projects.unshift(defaultProject); // Add at beginning as default
            }
        }
    ...
    static async listProjects(args) {
        try {
            ...
            if (projects.length === 0) {
                return ResponseBuilder.formatResponse({
                    ...
                    details: [
                        ...
                        '',
                        'To configure projects, either:',
                        '1. Set individual environment variables:',
                        '   - OPTIMIZELY_PROJECT_ID',
                        '   - OPTIMIZELY_API_KEY', 
                        '   - OPTIMIZELY_API_SECRET',
                        '',
                        '2. Or set OPTIMIZELY_PROJECTS with a JSON array:',
                        '   [{"name": "Project 1", "id": "...", "apiKey": "...", "apiSecret": "..."}]'
                    ].join('\n')
                });
            }
            ...
            projects.forEach((project, index) => {
                ...
                sections.push('');
                sections.push(`${index + 1}. **${project.name}**${defaultLabel}${dynamicLabel}`);
    ...
    static async handleGetSupport(args) {
        ...
        response += `hello@jaxondigital.com\n\n`;
        
        response += `**🐛 Report Issues**\n`;
        response += `GitHub: https://github.com/jaxondigital/jaxon-optimizely-dxp-mcp/issues\n\n`;
