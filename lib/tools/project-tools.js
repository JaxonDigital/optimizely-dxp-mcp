...
class ProjectTools {
    ...
    static getConfiguredProjects() {
        ...
        Object.keys(process.env).forEach(key => {
            ...
            // Check if this looks like our API key format OR a connection string
            // Must contain either: (id=, key=, secret=) OR (DefaultEndpointsProtocol=)
            ...
            try {
                ...
                // For self-hosted Azure, connection string replaces API credentials
                if (connectionString) {
                    // Self-hosted mode - connection string is all we need
                    // Project ID is optional for self-hosted (just for tracking)
                    if (!projectId) {
                ...
                // Add connection string for self-hosted Azure
                if (connectionString) {
