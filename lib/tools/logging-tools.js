...
class LoggingTools {
     ...
     */
    static async handleGetEdgeLogs(requestId, args) {
        ...
        try {
            ...
            return ResponseBuilder.internalError(requestId, 'Failed to get edge logs', error.message);
        }
    }
    ...
    static async getEdgeLogs(args) {
        ...
        console.error(`Getting edge log location for ${environment}`);

        // Build command
        let command = `Get-EpiEdgeLogLocation -ProjectId '${projectId}' -Environment '${environment}'`;
        ...
        if (result.stderr) {
            ...
            if (error) {
                // Check for specific edge log errors
                if (result.stderr.includes('push is not enabled') || 
    ...
    static formatEdgeLogLocation(data, environment) {
        ...
        let response = `${STATUS_ICONS.CLIPBOARD} **Edge/CDN Logs - ${environment}**\n\n`;
    ...
    static formatLogsNotEnabled(environment) {
        ...
        let response = `${STATUS_ICONS.WARNING} **Edge Logs Not Enabled**\n\n`;
        ...
        response += '2. Request Cloudflare log push activation\n';
        response += '3. Specify target environment(s)\n';
        response += '4. Configure log retention settings\n\n';
        response += '**Note:** This is a configuration setting at the DXP project level.\n';
