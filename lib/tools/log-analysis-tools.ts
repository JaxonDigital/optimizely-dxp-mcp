...
class LogAnalysisTools {
    ...
    static async analyzeSingleLogType(params: SingleLogTypeParams): Promise<LogAnalysisResult> {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(() => reject(new Error(`Log analysis timed out after ${TIMEOUT_MS / 1000} seconds`)), TIMEOUT_MS);
        });
     ...
     */
    static buildEmptyResponse(logType: string, structuredContent: boolean): any {
