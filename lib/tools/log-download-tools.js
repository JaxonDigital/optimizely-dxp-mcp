...
console.error('[LOG-DOWNLOAD-TOOLS] Module loaded at', new Date().toISOString());
...
**Status:** Container empty for the last ${args.daysBack || 7} days`);
            ...
            if (!args.skipConfirmation && !args.force && !args.isPartOfBatch) {
                ...
                // Return as confirmation required (error) to prevent auto-acceptance
                const confirmationResponse = `⛔ **STOP - USER CONFIRMATION REQUIRED** ⛔\n\n${fullResponse}\n\n🛑 **DO NOT PROCEED WITHOUT EXPLICIT USER CONFIRMATION** 🛑`;
                return ResponseBuilder.confirmationRequired(confirmationResponse);
            }
    ...
    static async downloadFromAllSelfHostedContainers(connectionString, containers, args, project) {
        ...
        if (!args.skipConfirmation && !args.force) {
            ...
            preview += `🛑 **DO NOT PROCEED WITHOUT EXPLICIT USER CONFIRMATION** 🛑`;
            
            return ResponseBuilder.confirmationRequired(preview);
        }
    ...
    static async handleDownloadAllLogs(args) {
        try {
            ...
            // ALWAYS show preview first (unless explicitly set to download immediately)
            // Only proceed if user explicitly confirms or force=true
            
            // DEBUG: Log the condition values for download all
            if (process.env.DEBUG === 'true') {
                console.error('[DOWNLOAD ALL PREVIEW DEBUG] Preview condition check:');
                console.error('  previewOnly:', args.previewOnly);
                console.error('  skipConfirmation:', args.skipConfirmation);
                console.error('  force:', args.force);
                console.error('  should show preview:', (args.previewOnly || (!args.skipConfirmation && !args.force)));
            }
            
            // Check for previewOnly FIRST - if set, always show preview and return success
            if (args.previewOnly || (!args.skipConfirmation && !args.force)) {
                ...
                for (const { logType, containerName } of availableLogTypes) {
                    try {
                        ...
                        const sasUrl = this.extractSasUrl(sasResponse);
                        if (sasUrl) {
                            // FAST preview check - just get first page without date filtering
                            const quickCheck = await this.listLogsPage(sasUrl, null);
                            if (quickCheck.logs && quickCheck.logs.length > 0) {
                                // Don't process all pages - just estimate based on first page
                                const estimatedTotal = quickCheck.nextMarker ? `${quickCheck.logs.length}+` : quickCheck.logs.length;
                                containersWithLogs.push({ logType, containerName, logCount: estimatedTotal });
                                OutputLogger.info(`✅ ${containerName}: Has logs`);
                            } else {
                ...
                if (containersWithLogs.length === 0) {
                    message += `## ⚠️ No Logs Found\n\n`;
                    message += `Checked ${availableLogTypes.length} container(s) but none contain logs for the last ${args.daysBack || 7} days.\n\n`;
                ...
                for (const { logType, containerName } of containersWithLogs) {
                    const containerSubfolder = this.getContainerSubfolderName(containerName);
                    ...
                    if (incremental) {
                        try {
                            // Get full list of logs for this container to check incremental
                            const sasResponse = await StorageTools.handleGenerateStorageSasLink({
                                apiKey: projectConfig.apiKey,
                                apiSecret: projectConfig.apiSecret,
                                projectId: projectConfig.projectId,
                            ...
                            const sasUrl = this.extractSasUrl(sasResponse);
                            if (sasUrl) {
                                const dateFilter = this.processDateFilters({ daysBack: args.daysBack || 7 });
                                const logs = await this.listLogs(sasUrl, dateFilter, containerName);
                                ...
                                if (manifestCheck.skippedFiles.length > 0) {
                                    ...
                                    message += `• Files to download: ${logs.length}\n`;
                                    totalFilesToDownload += logs.length;
                                }
                            }
                        } catch (error) {
                            // Silently continue if incremental check fails
                        }
                    }
                ...
                // If previewOnly mode, return success with the preview
                if (args.previewOnly) {
                    return ResponseBuilder.success(message);
                }
                ...
                // Return as confirmation required (error) to force Claude to stop
                const finalMessage = `⛔ **STOP - USER CONFIRMATION REQUIRED** ⛔\n\n${message}`;
                return ResponseBuilder.confirmationRequired(finalMessage);
            }
            
            // Download each log type - but check which ones have logs first
            OutputLogger.info('🔍 Checking which containers have logs...');
            const containersToDownload = [];
            ...
            for (const { logType, containerName } of availableLogTypes) {
                try {
                    ...
                    if (process.env.DEBUG === 'true') {
                        console.error(`[DEBUG] SAS URL for ${containerName}:`, sasUrl ? 'Generated successfully' : 'FAILED to generate');
                        if (!sasUrl) {
                            console.error(`[DEBUG] SAS response structure:`, JSON.stringify(sasResponse, null, 2));
                        }
                    }
                    
                    if (sasUrl) {
                        // Use processDateFilters to get proper date range with ALL time parameters
                        const dateFilter = this.processDateFilters(args);
                        
                        if (process.env.DEBUG === 'true') {
                            console.error(`[DEBUG] Checking ${containerName} with date filter:`, dateFilter);
                            
                            // TEMPORARY DEBUG: Also check without date filter to see if files exist
                            console.error(`[DEBUG] Testing without date filter for ${containerName}...`);
                            const logsNoFilter = await this.listLogs(sasUrl, {}, containerName, 10);
                            console.error(`[DEBUG] Without date filter: ${logsNoFilter.length} files found`);
                            if (logsNoFilter.length > 0) {
                                console.error(`[DEBUG] Sample file: ${logsNoFilter[0].name}`);
                            }
                        }
                        
                        // Always apply date filter - removed debug bypass
                        // FIXED: Don't limit to 5 files when checking for logs in "all" download
                        const logs = await this.listLogs(sasUrl, dateFilter, containerName);
                        
                        if (process.env.DEBUG === 'true') {
                            console.error(`[DEBUG] Found ${logs.length} logs in ${containerName}`);
                            if (logs.length > 0) {
                                console.error(`[DEBUG] First log: ${logs[0].name}`);
                            }
                        }
                        
                        if (logs.length > 0) {
                            containersToDownload.push({ logType, containerName });
                            OutputLogger.info(`✅ ${containerName}: Has logs, will download`);
                        } else {
                            OutputLogger.info(`⚠️  ${containerName}: No logs found, skipping`);
                        }
                    }
                } catch (error) {
                    OutputLogger.info(`❌ ${containerName}: Cannot access, skipping - ${error.message}`);
                    if (process.env.DEBUG === 'true') {
                        console.error(`[DEBUG] Error accessing ${containerName}:`, error);
                    }
                }
            }
            
            if (containersToDownload.length === 0) {
                return ResponseBuilder.success(
                    `## ⚠️ No Logs Found\n\n` +
                    `Checked ${availableLogTypes.length} container(s) but none contain logs for the last ${args.daysBack || 7} days.\n\n` +
                    `**Possible causes:**\n` +
                    `• Logs haven't been generated yet (can take up to 1 hour)\n` +
                    `• Logging may not be enabled for this environment\n` +
                    `• The containers may use a different date structure\n\n` +
                    `**Try:**\n` +
                    `• Checking a longer time range: \`daysBack: 30\`\n` +
                    `• Using a different environment: \`environment: "Integration"\`\n` +
                    `• Running \`discover_logs\` to see what's available`
                );
            }
            
            // Download each log type that has logs
            let allResults = [];
            OutputLogger.info(`🚀 Starting download of ${containersToDownload.length} log container(s)...`);
            
            for (const { logType, containerName } of containersToDownload) {
    ...
    static processDateFilters(args) {
        ...
        // Handle date range
        if (args.startDate && args.endDate) {
            // Support both date-only (2025/09/10) and datetime (2025-09-10T17:00) formats
