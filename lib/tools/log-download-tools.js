...
class LogDownloadTools {
    ...
    static async handleDownloadLogs(args) {
        try {
            ...
            const monitorProgress = args.monitor === true;
...
**Status:** Container empty for the last ${args.daysBack || 7} days`);
            ...
            if (incrementalInfo) {
                ...
                const totalSize = logsToDownload.reduce((sum, log) => sum + (log.size || 0), 0);
            ...
            const progressMonitor = new ProgressMonitor({
                ...
                totalBytes: logsToDownload.reduce((sum, log) => sum + (log.size || 0), 0),
            ...
            // DXP-84: Count total log entries in downloaded files
            let logCount = 0;
            if (downloadedCount > 0) {
                try {
                    logCount = await this.countLogEntries(downloadPath);
            ...
            if (typeof progressMonitor !== 'undefined' && progressMonitor) {
    ...
    static async downloadFromSelfHostedContainer(connectionString, containerName, args, project) {
        try {
            ...
            if (downloadedCount > 0) {
                try {
                    logCount = await this.countLogEntries(finalPath);
                    OutputLogger.info(`📊 Counted ${logCount} log entries across ${downloadedCount} files`);
                } catch (error) {
                    OutputLogger.warn(`⚠️ Could not count log entries: ${error.message}`);
                    // Continue without count - not critical
    ...
    static async listLogs(sasUrl, dateFilterObj, containerName = null, quickCheckLimit = null) {
        ...
        if (dateFilterObj && dateFilterObj.slot === true) {
            ...
            const samplePage = await this.listLogsPage(sasUrl, null);
            ...
            if (samplePage.logs.length > 0) {
                ...
                const slotBlob = samplePage.logs.find(log =>
    ...
    static async listLogsPage(sasUrl, marker) {
        return new Promise((resolve, reject) => {
            ...
            const req = https.get(listUrl, (response) => {
    ...
    /**
     * DXP-84: Count total log entries across all JSON files in a directory
     ...
     */
    static async countLogEntries(downloadPath) {
        ...
        const jsonFiles = await findJsonFiles(downloadPath);
     ...
     */
    static async handleDownloadAllLogsUserChoice(args) {
        try {
            // Resolve project configuration
            const resolved = ProjectTools.resolveCredentials(args);
            if (!resolved.success || !resolved.credentials) {
            ...
            for (const containerInfo of availableContainers) {
                ...
                try {
                    ...
                    const result = await Promise.race([previewOperation, previewTimeout]);
     ...
     */
    static async executeSequentialDownloads(args, projectConfig, projectName) {
        try {
            ...
            for (let i = 0; i < logContainersWithFiles.length; i++) {
                ...
                try {
                    ...
                    const result = await Promise.race([downloadPromise, downloadTimeout]);
            ...
            for (const result of results) {
                const status = result.success && (result.downloaded > 0 || result.skipped > 0) ? '✅' :
     ...
     */
    static async executeDirectSequentialDownloads(args, projectConfig, projectName) {
        try {
     ...
     */
    static async executeSimpleSequentialDownloads(args, projectConfig, projectName) {
        try {
            ...
            for (let i = 0; i < containersToTry.length; i++) {
                ...
                try {
                    ...
                    const result = await Promise.race([downloadPromise, timeout]);
     ...
     */
    static async downloadFromSpecificContainers(containerNames, args, projectConfig, projectName) {
        try {
     ...
     */
    static async handleDownloadAllLogsAutomatic(args) {
        try {
     ...
     */
    static async guideClientForAllLogs(args) {
        try {
     ...
     */
    static async quickLogCheck(sasUrl, args, containerName) {
        try {
     ...
     */
    static async handleBackgroundDownload(args, monitorProgress) {
        try {
            if (process.env.DEBUG === 'true') {
            ...
            try {
                ...
                const sasUrl = LogDiscoveryTools.extractSasUrl(sasResponse);
                ...
                if (sasUrl) {
                    ...
                    preview = {
                        ...
                        totalSize: logs.reduce((sum, log) => sum + (log.size || 0), 0),
     ...
     */
    static async runDownloadInBackground(downloadKey, args) {
        try {
