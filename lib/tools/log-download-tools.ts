...
const ResponseBuilder = require('../response-builder').default;
const ErrorHandler = require('../error-handler').default;
const OutputLogger = require('../output-logger').default;
const StorageTools = require('./storage-tools').default;
const ProjectTools = require('./project-tools').default;
const DownloadConfig = require('../download-config').default;
const downloadManager = require('../download-manager').default;
const ManifestManager = require('../manifest-manager').default;
const ProjectResolutionFix = require('./project-resolution-fix').default;
const SelfHostedStorage = require('../self-hosted-storage').default;
const ProgressMonitor = require('../progress-monitor').default;
...
class LogDownloadTools {
    ...
    static CONFIRMATION_TIMEOUT = 0; // TEMPORARILY DISABLED for testing - was 5000
     ...
     */
    static parseNaturalLanguageContainer(input: any) {
     ...
     */
    static async handleDownloadLogs(args: any) {
        // DXP-176: Declare variables outside try block so they're accessible in catch
        ...
        try {
            ...
            // DXP-3: Extract monitor and background parameters (default: false for opt-in behavior)
            monitorProgress = args.monitor === true;
            const backgroundMode = args.background === true;

            if (process.env.DEBUG === 'true') {
                if (monitorProgress) console.error('[DEBUG] Progress monitoring enabled');
                if (backgroundMode) console.error('[DEBUG] Background mode enabled - will return immediately');
            }

            // DXP-3: If background mode, delegate to background download handler
            if (backgroundMode) {
                return await this.handleBackgroundDownload(args, monitorProgress);
            }
            ...
            if (process.env.DEBUG === 'true') {
                ...
                console.error('[DEBUG] All configured projects:', allProjects.map((p: any) => ({
                    name: p.name,
            ...
            // Use args.projectName (set by withProjectResolution wrapper) as primary source
            // Fall back to resolution.project.name if needed
            let projectName = args.projectName || (resolution.project ? resolution.project.name : 'Unknown');
            ...
            // Handle "all" option - guide AI client to use individual requests
            if (args.logType === 'all') {
                return this.guideClientForAllLogs(args);
            }
            ...
            let dateFilter = args.prefetchedDateFilter || this.processDateFilters(args);
            if (dateFilter) {
                OutputLogger.info(`📅 Date filter: ${dateFilter.description}`);
            }

            // Add slot parameter to dateFilter for filtering logic
            if (dateFilter && args.slot !== undefined) {
                dateFilter.slot = args.slot;
            } else if (!dateFilter && args.slot !== undefined) {
                // Create dateFilter object if it doesn't exist but we have slot parameter
                dateFilter = { slot: args.slot };
            }

            if (args.slot) {
                OutputLogger.info(`🎯 Slot filter: Deployment slot logs only (/SLOTS/SLOT/)`);
            } else if (args.slot === false) {
                OutputLogger.info(`📍 Slot filter: Production logs only (excluding /SLOTS/ paths)`);
            }
            ...
            if (args.previewOnly) {
                ...
                // DXP-22: Add 'slot' subfolder for slot logs to avoid mixing with production
                if (args.slot === true) {
                    downloadPath = path.join(basePath, 'slot', containerSubfolder);
                } else {
                ...
                // DXP-22: Or /logs/project/slot/[container-name]/ for slot logs
                const containerSubfolder = this.getContainerSubfolderName(containerName);

                if (args.slot === true) {
                    downloadPath = path.join(validated.path, 'slot', containerSubfolder);
                } else {
            ...
            OutputLogger.info(`\n${'='.repeat(60)}`);
            if (args.slot === true) {
                OutputLogger.info(`📁 DOWNLOAD LOCATION (DEPLOYMENT SLOT LOGS):`);
            } else {
...
**Status:** Container empty for the last ${args.daysBack || 7} days`);
            ...
            if (args.environment === 'Production' && logs.length > 0) {
                ...
                if (logDateMatch) {
                    ...
                    const daysSinceLastLog = Math.floor((new Date().getTime() - logDate.getTime()) / (1000 * 60 * 60 * 24));
            ...
            const totalLogSize = logs.reduce((sum: any, log: any) => sum + (log.size || 0), 0);
            const logFiles = logs.filter((log: any) => log.fileType === 'log');
            const otherFiles = logs.filter((log: any) => log.fileType === 'other');
            ...
            if (incremental) {
                ...
                // Always set incrementalInfo to show stats, even if 0 files skipped
                incrementalInfo = {
                    skippedFiles: manifestCheck.skippedFiles.length,
                    toDownload: manifestCheck.filesToDownload.length,
                    skippedSize: manifestCheck.skippedFiles.reduce((sum: any, f: any) => sum + (f.size || 0), 0),
                    manifest: manifestCheck.manifest,
                    filesToDownload: manifestCheck.filesToDownload,
                    skippedFilesList: manifestCheck.skippedFiles
                };

                OutputLogger.info(`[INCREMENTAL] Detection complete: ${manifestCheck.skippedFiles.length} files up-to-date, ${manifestCheck.filesToDownload.length} files to download`);
            }
            ...
            if (args.previewOnly) {
                ...
                // Build preview message with PROMINENT incremental stats at the top
                let previewMessage = '';

                // DXP-52: Show incremental stats prominently at the top of preview
                if (incrementalInfo) {
                    // Use the same prominent display as skipConfirmation mode
                    if (incrementalInfo.skippedFiles > 0) {
                        previewMessage = `✨ **Smart Incremental Download Active**\n\n`;
                    } else {
                        previewMessage = `📊 **Incremental Download Status**\n\n`;
                    }
                    previewMessage += `**Optimization Summary:**\n`;
                    previewMessage += `• Files already up-to-date: ${incrementalInfo.skippedFiles}\n`;
                    previewMessage += `• Files to download: ${incrementalInfo.toDownload}\n`;
                    previewMessage += `• Bandwidth saved: ${ManifestManager.formatBytes(incrementalInfo.skippedSize || 0)}\n`;

                    const totalSizeWithSkipped = totalLogSize + (incrementalInfo.skippedSize || 0);
                    const savedPct = totalSizeWithSkipped > 0 ?
                        Math.round((incrementalInfo.skippedSize / totalSizeWithSkipped) * 100) : 0;
                    if (savedPct > 0) {
                        previewMessage += `• Efficiency: ${savedPct}% reduction\n`;
                    }
                    previewMessage += `\n---\n\n`;
                }

                // Add the regular preview content
                previewMessage += this.formatLogDownloadConfirmation(
                    logs,
                    totalLogSize,
                    containerName,
                    logType,
            ...
            // Check if all files are already downloaded before asking for confirmation
            if (!args.skipConfirmation && !args.force && !args.isPartOfBatch) {
                // If incremental info shows no files to download, skip confirmation
                if (incrementalInfo && (incrementalInfo.toDownload === 0 ||
                    (incrementalInfo.filesToDownload && incrementalInfo.filesToDownload.length === 0))) {

                    let message = `# 📋 Download Status - All Files Up to Date\n\n`;
                    message += `**Container:** ${containerName}\n`;
                    message += `**Environment:** ${args.environment}\n`;
                    message += `**Files Found:** ${logs.length}\n`;
                    message += `**Files Already Downloaded:** ${incrementalInfo.skippedFiles || logs.length}\n`;
                    message += `**Files to Download:** 0\n\n`;
                    message += `✅ **All ${logs.length} log files are already up-to-date in your local folder.**\n\n`;
                    message += `No download needed - all files are current and unchanged.\n\n`;
                    message += `**Download Path:** ${downloadPath}`;
                ...
                const confirmMessage = this.formatLogDownloadConfirmation(
                    ...
                    downloadPath,
                    dateFilter,  // Use processed dateFilter, not args.dateFilter
                    incrementalInfo as any  // Pass incremental info to preview
                );
            ...
            if (!incrementalInfo) {
                ...
                logsToDownload = manifestCheck.filesToDownload.map((f: any) => {
                    // Map back to original log format
                    const originalLog = logs.find((l: any) => l.name === f.name);
                    return originalLog || f;
                });

                if (skippedFiles.length > 0) {
                    OutputLogger.info(`✨ Smart download: Skipping ${skippedFiles.length} unchanged log files`);
                    OutputLogger.info(`   Bandwidth saved: ${ManifestManager.formatBytes(skippedFiles.reduce((sum: any, f: any) => sum + (f.size || 0), 0))}`);
                }
                }
            } else {
                ...
                if (incrementalInfo.filesToDownload) {
                    logsToDownload = incrementalInfo.filesToDownload.map((f: any) => {
                        const originalLog = logs.find((l: any) => l.name === f.name);
                        return originalLog || f;
                    });
                }
            ...
            // Build initial response with incremental stats prominently displayed
            let initialResponse = '';
            if (incrementalInfo) {
                // Always show incremental stats, even if 0 files are skipped
                if (incrementalInfo.skippedFiles > 0) {
                    initialResponse = `✨ **Smart Incremental Download Active**\n\n`;
                } else {
                    initialResponse = `📊 **Incremental Download Status**\n\n`;
                }
                initialResponse += `**Optimization Summary:**\n`;
                initialResponse += `• Files already up-to-date: ${incrementalInfo.skippedFiles}\n`;
                initialResponse += `• Files to download: ${logsToDownload.length}\n`;
                initialResponse += `• Bandwidth saved: ${ManifestManager.formatBytes(incrementalInfo.skippedSize || 0)}\n`;

                const totalSize = logsToDownload.reduce((sum: any, log: any) => sum + (log.size || 0), 0);
                const savedPct = (totalSize + incrementalInfo.skippedSize) > 0 ?
                    Math.round((incrementalInfo.skippedSize / (totalSize + incrementalInfo.skippedSize)) * 100) : 0;
                if (savedPct > 0) {
                    initialResponse += `• Efficiency: ${savedPct}% reduction\n`;
                }
                initialResponse += `\n`;
            }
            ...
            // Register download for tracking and cancellation (skip if already registered by background handler)
            let downloadKey;
            ...
            if (args.__backgroundDownload && args.__downloadKey) {
                ...
                downloadKey = downloadManager.registerDownload({
                    projectName,
                    containerName,
                    environment: args.environment,
                    dateRange: this.describeDateRange(args),
            ...
            progressMonitor = new ProgressMonitor({
                ...
                totalBytes: logsToDownload.reduce((sum: any, log: any) => sum + (log.size || 0), 0),
            ...
            let failedCount = 0;
            let totalSize = 0;
            const downloadedFilePaths = []; // DXP-130: Track downloaded files for accurate logCount
            ...
            for (let i = 0; i < logsToDownload.length; i += BATCH_SIZE) {
                ...
                const batchPromises = batch.map(async (log: any) => {
                    try {
                        ...
                        const size: any = await this.downloadLogFile(log.url, localPath, log.name);
                        downloadedCount++;
                        totalSize += (size as any);

                        // DXP-3: Update progress monitor if enabled
                        progressMonitor.update(downloadedCount, totalSize, log.name);

                        // Update download manager progress
                        const progress = Math.round((downloadedCount / logs.length) * 100);
                        downloadManager.updateProgress(downloadKey, progress);

                        // Only show individual file messages if monitoring is disabled
                        if (!monitorProgress) {
                        ...
                        return { success: true, size, localPath }; // DXP-130: Return localPath
                    } catch (error: any) {
                        failedCount++;
                ...
                // DXP-130: Collect downloaded file paths
                for (const result of results) {
                    if (result.success && result.localPath) {
                        downloadedFilePaths.push(result.localPath);
                    }
                }

                // Log batch progress (only show if monitoring disabled)
                if (!monitorProgress) {
            ...
            // Save even if no new downloads to update timestamps and ensure persistence
            if (incrementalInfo && incrementalInfo.manifest) {
                await ManifestManager.saveManifest(downloadPath, incrementalInfo.manifest);
                if (downloadedCount > 0) {
                    OutputLogger.info('📝 Manifest updated with new downloads');
                } else {
                    OutputLogger.debug('📝 Manifest timestamps updated');
                }
            }

            // DXP-84/DXP-130: Count total log entries in downloaded files
            let logCount = 0;
            if (downloadedCount > 0) {
                try {
                    // DXP-130: Pass downloaded file paths to count only new files
                    logCount = await this.countLogEntries(downloadPath, downloadedFilePaths as any);
                    OutputLogger.info(`📊 Counted ${logCount} log entries across ${downloadedCount} files`);
                } catch (error: any) {
                    OutputLogger.warn(`⚠️ Could not count log entries: ${error.message}`);
            ...
            // DXP-190: Make download path VERY prominent
            response += `📁 **Files Location:**\n`;
            response += `\`\`\`\n${downloadPath}\n\`\`\`\n`;
            response += `✅ Verified ${actualFilesOnDisk} files on disk\n\n`;

            // Prepend initial response with incremental stats if we have them
            if (initialResponse) {
            ...
            if (skippedFiles.length > 0) {
                const savedSize = skippedFiles.reduce((sum: any, f: any) => sum + (f.size || 0), 0);
            ...
            // DXP-190: Verify files actually exist on disk before marking complete
            let actualFilesOnDisk = 0;
            try {
                const allFiles = await fs.readdir(downloadPath, { recursive: true });
                actualFilesOnDisk = allFiles.filter((f: string) => f.endsWith('.json')).length;
                OutputLogger.info(`✅ Verified ${actualFilesOnDisk} files written to disk at ${downloadPath}`);

                if (actualFilesOnDisk === 0 && downloadedCount > 0) {
                    throw new Error(`Download reported ${downloadedCount} files but 0 files found at ${downloadPath}`);
                }
            } catch (error: any) {
                OutputLogger.warn(`⚠️ File verification warning: ${error.message}`);
                // Continue anyway - files may exist but directory structure different
            }
            ...
            const result = {
                ...
                downloadPath,
                actualFilesOnDisk  // DXP-190: Include verified file count
            };
            ...
            const structuredData = {
                ...
                totalSize: this.formatBytes(totalSize),
                dateFilter: args.dateFilter || null,
                isSparseLogging: isSparseLogging,
                logCount: logCount,  // DXP-84: Total number of log entries across all files
                actualFilesOnDisk: actualFilesOnDisk  // DXP-190: Verified file count
            };
            ...
            return ResponseBuilder.successWithStructuredData(structuredData, response);
            
        } catch (error: any) {
            // DXP-3: Mark download as failed in progress monitor if it exists
     ...
     */
    static async handleSelfHostedLogDownload(args: any, project: any) {
        try {
            ...
            const logContainers = containers.filter((c: any) =>
            ...
            if (logContainers.length === 0) {
                return ResponseBuilder.success(
                    ...
                    containers.map((c: any) => `• ${c.name}`).join('\n') +
            ...
            if (!args.containerName && !args.logType) {
                return ResponseBuilder.success(
                    ...
                    logContainers.map((c: any, i: any) => `${i + 1}. 📦 ${c.name}`).join('\n') +
                    `\n\n**To download logs:**\n` +
            ...
            if (!targetContainer && args.logType) {
                ...
                if (args.logType === 'all') {
                    ...
                    targetContainer = logContainers.find((c: any) =>
            ...
            if (!targetContainer) {
                return ResponseBuilder.error(
                    ...
                    `Available containers: ${logContainers.map((c: any) => c.name).join(', ')}`
                );
            }
     ...
     */
    static async downloadFromSelfHostedContainer(connectionString: any, containerName: any, args: any, project: any) {
        try {
            // Get download path
            ...
            // DXP-86: Add slot parameter to dateFilter for filtering logic
            if (dateFilter && args.slot !== undefined) {
                dateFilter.slot = args.slot;
            } else if (!dateFilter && args.slot !== undefined) {
                // Create dateFilter object if it doesn't exist but we have slot parameter
            ...
            if (process.env.DEBUG === 'true') {
                ...
                console.error(`[SELF-HOSTED] Slot parameter:`, args.slot);
            ...
            if (args.previewOnly) {
                ...
                // DXP-52: Show incremental stats prominently at the top of preview (self-hosted)
                if (incrementalInfo) {
                    // Use the same prominent display as skipConfirmation mode
                    if (incrementalInfo.skippedFiles > 0) {
                        preview = `✨ **Smart Incremental Download Active**\n\n`;
                    } else {
                        preview = `📊 **Incremental Download Status**\n\n`;
                    }
                    preview += `**Optimization Summary:**\n`;
                    preview += `• Files already up-to-date: ${incrementalInfo.skippedFiles}\n`;
                    preview += `• Files to download: ${incrementalInfo.toDownload}\n`;
                    preview += `• Bandwidth saved: ${ManifestManager.formatBytes(incrementalInfo.skippedSize || 0)}\n`;

                    const totalSizeWithSkipped = totalLogSize + (incrementalInfo.skippedSize || 0);
                    const savedPct = totalSizeWithSkipped > 0 ?
                        Math.round((incrementalInfo.skippedSize / totalSizeWithSkipped) * 100) : 0;
                    if (savedPct > 0) {
                        preview += `• Efficiency: ${savedPct}% reduction\n`;
                    }
                    preview += `\n---\n\n`;
                }

                // Add the regular preview content
                preview += this.formatLogDownloadConfirmation(
                    logs,
                    totalLogSize,
                    containerName,
                    args.logType || 'logs',
            ...
            for (let i = 0; i < logsToDownload.length; i += BATCH_SIZE) {
                ...
                const batchPromises = batch.map(async (log: any) => {
                    ...
                    try {
                        const size: any = await this.downloadLogFile(blobUrl, localPath, log.name);
                        downloadedCount++;
                        totalSize += size;
            ...
            if (downloadedCount > 0) {
                try {
                    // DXP-130: Pass downloaded file paths to count only new files
                    logCount = await this.countLogEntries(finalPath, downloadedFilePaths);
                    OutputLogger.info(`📊 Counted ${logCount} log entries across ${downloadedCount} files`);
                } catch (error: any) {
                    OutputLogger.warn(`⚠️ Could not count log entries: ${error.message}`);
                    // Continue without count - not critical
            ...
            // Build response with incremental stats prominently displayed
            let responseMessage = '';
            if (incrementalInfo && incrementalInfo.skippedFiles > 0) {
                responseMessage = `✨ **Smart Incremental Download Results**\n\n`;
                responseMessage += `**Optimization Applied:**\n`;
                responseMessage += `• Files already up-to-date: ${incrementalInfo.skippedFiles}\n`;
                responseMessage += `• Files downloaded: ${downloadedCount}\n`;
                responseMessage += `• Bandwidth saved: ${ManifestManager.formatBytes(incrementalInfo.skippedSize || 0)}\n`;
                const savedPct = (totalSize + incrementalInfo.skippedSize) > 0 ?
                    Math.round((incrementalInfo.skippedSize / (totalSize + incrementalInfo.skippedSize)) * 100) : 0;
                responseMessage += `• Efficiency gain: ${savedPct}%\n\n`;
            }

            responseMessage += `✅ **Log Download Complete**\n\n`;
            responseMessage += `**Container:** ${containerName}\n`;
            responseMessage += `**Files Downloaded:** ${downloadedCount}\n`;
            if (incrementalInfo && incrementalInfo.skippedFiles > 0) {
                responseMessage += `**Files Skipped:** ${incrementalInfo.skippedFiles} (already up-to-date)\n`;
            }
            responseMessage += `**Total Size:** ${(totalSize / (1024 * 1024)).toFixed(2)} MB\n`;
            if (incrementalInfo && incrementalInfo.skippedSize > 0) {
                responseMessage += `**Bandwidth Saved:** ${ManifestManager.formatBytes(incrementalInfo.skippedSize)}\n`;
            }
            responseMessage += `**Location:** ${finalPath}/\n\n`;
            responseMessage += `Logs are organized by date and ready for analysis.`;

            // DXP-84: Add structured data with log count
            const structuredData = {
                containerName: containerName,
                downloadPath: finalPath,
                downloadedCount: downloadedCount,
                skippedCount: incrementalInfo?.skippedFiles || 0,
                totalSizeBytes: totalSize,
                totalSize: this.formatBytes(totalSize),
                logCount: logCount  // DXP-84: Total number of log entries
            };

            return ResponseBuilder.successWithStructuredData(structuredData, responseMessage);
     ...
     */
    static async downloadFromAllSelfHostedContainers(connectionString: any, containers: any, args: any, project: any) {
     ...
     */
    static async handleDownloadAllLogs(args: any) {
        try {
            ...
            const containerCheckPromises = availableLogTypes.map(async ({ logType, containerName }) => {
                // Add timeout wrapper to prevent hanging on individual container checks
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => reject(new Error(`Container check timeout for ${containerName}`)), 60000) // 60 second timeout
                );
                ...
                const checkPromise = (async () => {
                    ...
                    if (sasUrl) {
                        ...
                        // DXP-46 FIX: Don't apply quick check limit for recent date ranges
                        // The quick check limit causes issues when looking for today's logs in large containers
                        // because logs are ordered by path (date), and today's logs are at the END, not beginning
                        const isLookingForRecentLogs = dateFilter && dateFilter.startDate &&
                            ((new Date().getTime() - dateFilter.startDate.getTime()) < 48 * 60 * 60 * 1000); // Within last 48 hours

                        // Apply limit for preview OR when we're going to show a preview (not direct download)
                        // BUT skip the limit if looking for recent logs (they're at the end of the list)
                        if ((isPreviewMode || isShowingPreview) && dateFilter && dateFilter.startDate && dateFilter.endDate && !isLookingForRecentLogs) {
                        ...
                        } else if (isLookingForRecentLogs && (isPreviewMode || isShowingPreview)) {
                            // For recent logs, we need to check ALL files since they're at the end
                            if (process.env.DEBUG === 'true') {
                                console.error(`[DXP-46 FIX] Looking for recent logs (within 48h) - skipping quick check limit to ensure we find them`);
                            }
                        }
                        ...
                        if (logs && logs.length > 0) {
                            ...
                            const totalSize = logs.reduce((sum: any, log: any) => sum + (log.size || 0), 0);
                ...
                // Race the check promise against the timeout
                try {
                    return await Promise.race([checkPromise, timeoutPromise]);
                } catch (error: any) {
                    if (process.env.DEBUG === 'true') {
                        console.error(`[PARALLEL] ❌ ${containerName}: ${error.message}`);
                    }
                    return null; // Return null on timeout or error so other containers can continue
                }
            });
                ...
                containersWithLogs = containerResults.filter(result => result !== null) as any;
            ...
            if (containersWithLogs.length === 0) {
                ...
                if (args.startDateTime && args.endDateTime) {
                    ...
                    // Handle 0 days (today) correctly - DXP-46
                    if (args.daysBack === 0) {
                        timeRangeStr = 'today';
                    } else {
            ...
            if (args.previewOnly || (!args.skipConfirmation && !args.force)) {
                ...
                const overallTotalFiles = containersWithLogs.reduce((sum: any, c: any) => sum + c.logCount, 0);
                const overallTotalSize = containersWithLogs.reduce((sum: any, c: any) => sum + c.totalSize, 0);
                    ...
                    if (incremental && container.logs) {
                        try {
                            ...
                            if (manifestCheck.skippedFiles.length > 0) {
                                const skippedSize = manifestCheck.skippedFiles.reduce((sum: any, f: any) => sum + (f.size || 0), 0);
            ...
            for (const { logType, result } of allResults) {
                const resultDetails: any = { logType, status: 'unknown', files: 0, skipped: 0, size: '0 B' };
            ...
            if (hasErrors) {
                ...
                if (args.startDateTime && args.endDateTime) {
                    const startDate = new Date(args.startDateTime);
                    const endDate = new Date(args.endDateTime);
                    const rangeHours = ((endDate as any) - (startDate as any)) / (1000 * 60 * 60);
                    const rangeDays = rangeHours / 24;
     ...
     */
    static showAvailableContainers(args: any, containers: any, requestedLogType: any) {
        ...
        const otherContainers = containers.filter((c: any) => !logContainers.includes(c));
     ...
     */
    static async showLogTypeSelection(args: any) {
     ...
     */
    static formatLogDownloadConfirmation(logs: any, totalSize: any, containerName: any, logType: any, environment: any, downloadPath: any, dateFilter: any, incrementalInfo: any = null) {
     ...
     */
    static async determineLogDownloadPath(args: any, projectName: any, containerName: any) {
        // Get base path first
        let basePath;
     ...
     */
    static getContainerSubfolderName(containerName: any) {
     ...
     */
    static async determineLogDownloadPathLegacy(_args: any, projectName: any) {
     ...
     */
    static describeDateRange(args: any) {
        if (args.daysBack) {
     ...
     */
    static async listLogsWithPrefix(sasUrl: any, prefix: any) {
     ...
     */
    static generateDatePrefixes(_startDate: any, _endDate: any) {
        // DXP-46 FIX: Remove hardcoded subscription - this optimization
        // only works for one specific project. Disable prefix optimization
        // until we can make it dynamic per project.

        // Return null to trigger full scan with proper date filtering
        // This is slower but works for all projects
        return null;

        /* Original code with hardcoded values - DO NOT USE
        const prefixes = [];
        ...
        while (current <= endDate) {
            ...
            // PROBLEM: Hardcoded subscription/resource group only works for one project!
     ...
     */
    static async listLogs(sasUrl: any, dateFilterObj: any, containerName: any = null, quickCheckLimit: any = null) {
        const allLogs = [];

        // DXP-22/DXP-86: SLOT PREFIX OPTIMIZATION - When slot=true, use prefix to only fetch slot logs
        if (dateFilterObj && dateFilterObj.slot === true) {
            console.error(`[SLOT FILTER] slot=true detected - searching for deployment slot logs (/SLOTS/SLOT/)`);
            console.error(`[SLOT FILTER] Scanning first page to discover slot path pattern...`);

            // Quick scan of first page to find resource ID pattern
            const samplePage: any = await this.listLogsPage(sasUrl, null);
            console.error(`[SLOT FILTER] First page contains ${samplePage.logs.length} blobs`);

            if (samplePage.logs.length > 0) {
                // DXP-86: Show sample paths for debugging
                if (process.env.DEBUG === 'true') {
                    console.error(`[SLOT FILTER] Sample blob paths from first page:`);
                    for (let i = 0; i < Math.min(3, samplePage.logs.length); i++) {
                        console.error(`  ${i + 1}. ${samplePage.logs[i].name.substring(0, 150)}...`);
                    }
                }

                // DXP-86: Use case-insensitive match for slot path detection
                const slotBlob = samplePage.logs.find((log: any) =>
                    log.name.toUpperCase().includes('/SLOTS/SLOT/')
                );

                if (slotBlob) {
                    // Extract everything before /SLOTS/SLOT/ (case-insensitive)
                    const match = slotBlob.name.match(/^(.*?\/SLOTS\/SLOT\/)/i);
                    if (match) {
                        const slotPrefix = match[1];
                        console.error(`[SLOT FILTER] ✓ Found slot path pattern in first page`);
                        console.error(`[SLOT FILTER] Slot prefix: ${slotPrefix.substring(0, 80)}...`);
                        console.error(`[SLOT FILTER] Using optimized prefix query (much faster than full scan)`);

                        // Use prefix query to get only slot logs
                        const slotLogs = await this.listLogsWithPrefix(sasUrl, slotPrefix);
                        console.error(`[SLOT FILTER] ✓ Prefix query found ${slotLogs.length} slot log files`);

                        // Apply date filtering on the slot logs
                        return this.filterLogsByDate(slotLogs, sasUrl, dateFilterObj, containerName);
                    }
                } else {
                    console.error(`[SLOT FILTER] ✗ No slot logs found in first page (${samplePage.logs.length} blobs checked)`);
                    console.error(`[SLOT FILTER] This could mean:`);
                    console.error(`  - Slot logs haven't been created yet (deployment not run)`);
                    console.error(`  - Slot logs are deeper in pagination`);
                    console.error(`[SLOT FILTER] Falling back to full container scan with slot filtering...`);
                }
            } else {
                console.error(`[SLOT FILTER] ✗ First page is empty - container might be empty`);
            }

            console.error(`[SLOT FILTER] Performing full container scan (slower) with slot path filtering...`);
        }
        ...
        do {
            ...
            const pageLogs: any = await this.listLogsPage(sasUrl, nextMarker);
     ...
     */
    static async listLogsPage(sasUrl: any, marker: any) {
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                req.destroy();
                reject(new Error(`Container listing timeout after 60 seconds`));
            }, 60000); // 60 second timeout

            const req = https.get(listUrl, (response: any) => {
            ...
            // Set socket timeout as additional safety
            req.setTimeout(60000, () => {
                req.destroy();
                reject(new Error(`Socket timeout after 60 seconds for container listing`));
            });
        });
    }
     ...
     */
    static filterLogsByDate(allLogs: any, sasUrl: any, dateFilterObj: any, containerName: any = null) {
        ...
        const slotFilter = dateFilterObj?.slot;

        // Debug: Log what date filter we're using
        if (startDate || endDate || dateFilter || slotFilter !== undefined) {
            console.error(`[FILTER ACTIVE]`);
            ...
            console.error(`  slotFilter: ${slotFilter !== undefined ? (slotFilter ? 'deployment slot only' : 'production only') : 'none'}`);
        ...
        for (const log of allLogs) {
            ...
            // Apply slot filter if specified (DXP-22/DXP-86)
            if (slotFilter !== undefined) {
                const nameUpper = name.toUpperCase();
                if (slotFilter === true) {
                    // slot=true: Only include deployment slot logs (/SLOTS/SLOT/)
                    // DXP-86: Use case-insensitive matching to handle different path casings
                    if (!nameUpper.includes('/SLOTS/SLOT/')) {
                        filesSkippedBySlot++;
                        continue;
                    }
                } else if (slotFilter === false) {
                    // slot=false (default): Exclude ALL slot logs (any /SLOTS/ path)
                    // DXP-86: Use case-insensitive matching
                    if (nameUpper.includes('/SLOTS/')) {
                        filesSkippedBySlot++;
                        continue;
                    }
                }
            ...
            if (startDate && endDate) {
                ...
                if (logDateTime) {
                    // DXP-85: Special handling for hourly log files (PT1H.json)
                    // These files contain logs for the entire hour, not just the timestamp in the path
                    const isHourlyFile = name.includes('PT1H.json');
                    let logEndDateTime = logDateTime;

                    if (isHourlyFile) {
                        // For hourly files, the end time is 1 hour after the start
                        logEndDateTime = new Date(logDateTime.getTime() + 60 * 60 * 1000);
                    }
                    ...
                    if (process.env.DEBUG === 'true' || process.env.LOG_DATE_FILTER === 'true') {
                        if (name.includes('2025')) {
                            ...
                            if (isHourlyFile) {
                                console.error(`  Hourly file end: ${logEndDateTime.toISOString()}`);
                            }
                            console.error(`  Filter range: ${startDate.toISOString()} to ${endDate.toISOString()}`);
                            if (isHourlyFile) {
                                // Check for overlap: hour end >= filter start AND hour start <= filter end
                                const hasOverlap = logEndDateTime >= startDate && logDateTime <= endDate;
                                console.error(`  Hourly overlap: ${hasOverlap}`);
                            } else {
                    ...
                    // DXP-85: For hourly files, check if ANY part of the hour overlaps with the requested range
                    // Overlap exists if: hour_end >= filter_start AND hour_start <= filter_end
                    if (isHourlyFile) {
                        if (logEndDateTime < startDate || logDateTime > endDate) {
                            filesSkippedByDate++;
                            if (process.env.DEBUG === 'true' || filesSkippedByDate <= 5) {
                                console.error(`[DXP-85 SKIP] Skipping hourly file ${name} (no overlap with range)`);
                            }
                            continue;
                        }
                    } else {
        ...
        if ((startDate && endDate) || dateFilter || slotFilter !== undefined) {
            if (process.env.DEBUG === 'true' || process.env.LOG_DATE_FILTER === 'true' || filteredLogs.length === 0) {
                ...
                console.error(`  Files skipped (slot filter): ${filesSkippedBySlot}`);
                ...
                if (filteredLogs.length === 0 && allLogs.length > 0) {
                    ...
                    allLogs.slice(0, 10).forEach((log: any) => {
     ...
     */
    static parseLogListXml_DEPRECATED(xml: any, sasUrl: any, dateFilterObj: any) {
        const logs = [];
        const baseUrl = sasUrl.split('?')[0];
        const sasToken = sasUrl.split('?')[1];
     ...
     */
    static async downloadLogFile(fileUrl: any, localPath: any, displayName: any) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                req.destroy();
                reject(new Error(`Download timeout after 60 seconds for ${displayName}`));
            }, 60000); // 60 second timeout

            const req = https.get(fileUrl, (response: any) => {
                clearTimeout(timeout);
            ...
            req.setTimeout(60000, () => {
                req.destroy();
                reject(new Error(`Socket timeout after 60 seconds for ${displayName}`));
            });
        });
    }
     ...
     */
    static extractContainerList(result: any) {
        let text = '';
     ...
     */
    static extractSasUrl(result: any) {
        try {
            let textToSearch = '';
    ...
    /**
     * DXP-84/DXP-130: Count total log entries across JSON files
     * Each line in a JSON log file represents one log entry
     * @param {string} downloadPath - Path to directory containing log files
     * @param {string[]} [filePaths] - Optional array of specific file paths to count (DXP-130: fixes counting old files)
     * @returns {Promise<number>} Total number of log entries
     */
    static async countLogEntries(downloadPath: any, filePaths: any = null) {
        ...
        // DXP-130: If specific files provided, only count those
        if (filePaths && Array.isArray(filePaths) && filePaths.length > 0) {
            jsonFiles = filePaths;
        } else {
            // Fall back to scanning all .json files recursively
            const findJsonFiles = async (dir: any): Promise<string[]> => {
                const files = [];
            ...
            jsonFiles = await findJsonFiles(downloadPath);
        }
        ...
        for (const filePath of jsonFiles) {
            try {
                ...
                if (process.env.DEBUG === 'true') {
                    console.error(`[DEBUG] Could not count lines in ${filePath}: ${error.message}`);
                }
            }
        }
     ...
     */
    static parseFlexibleDate(dateStr: any) {
     ...
     */
    static parseTimeString(timeStr: any) {
     ...
     */
    static convertToUTC(baseDate: any, hours: any, minutes: any, timezoneOffset: any) {
     ...
     */
    static processDateFilters(args: any) {
        
        
        // DXP-20/DXP-88: Handle datetime parameters with smart defaults
        // Supported formats:
        ...
        if (args.startDateTime || args.endDateTime) {
            ...
            // DXP-88: Smart defaults - allow single startDateTime or endDateTime
            if (args.startDateTime && !args.endDateTime) {
                // Only startDateTime provided → default endDateTime to NOW
                startDate = new Date(args.startDateTime);

                if (isNaN(startDate.getTime())) {
                    throw new Error(`Invalid startDateTime format: "${args.startDateTime}". Use ISO 8601 format (e.g., "2025-10-08T14:00:00Z")`);
                }

                endDate = new Date(); // Now
                description = `${args.startDateTime} to now`;

            } else if (args.endDateTime && !args.startDateTime) {
                // Only endDateTime provided → default startDateTime to 7 days before
                endDate = new Date(args.endDateTime);

                if (isNaN(endDate.getTime())) {
                    throw new Error(`Invalid endDateTime format: "${args.endDateTime}". Use ISO 8601 format (e.g., "2025-10-08T14:00:00Z")`);
                }

                startDate = new Date(endDate.getTime() - (7 * 24 * 60 * 60 * 1000)); // 7 days before
                description = `7 days before ${args.endDateTime}`;

            } else {
                // Both provided (original behavior)
                startDate = new Date(args.startDateTime);
                endDate = new Date(args.endDateTime);

                if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                    throw new Error(`Invalid ISO 8601 format. Use format like "2025-10-08T14:00:00Z" or "2025-10-08T14:00:00-05:00"`);
                }

                description = `${args.startDateTime} to ${args.endDateTime}`;
            }

            // Validate date range: startDateTime must be before endDateTime
            if (startDate >= endDate) {
                const startStr = args.startDateTime || `calculated (7 days before ${args.endDateTime})`;
                const endStr = args.endDateTime || 'now';
                throw new Error(`Invalid date range: startDateTime (${startStr}) must be before endDateTime (${endStr})`);
            }

            // Validate future dates: cannot download logs from the future
            const now = new Date();
            if (startDate > now) {
                throw new Error(`startDateTime cannot be in the future: "${args.startDateTime}"`);
            }
            if (endDate > now && args.endDateTime) {
                throw new Error(`endDateTime cannot be in the future: "${args.endDateTime}"`);
            }
        ...
        // Handle date range with smart defaults
        if (args.startDate || args.endDate) {
            let startDate, endDate;
            let description;

            // Smart defaults: allow single startDate or endDate
            if (args.startDate && !args.endDate) {
                // Only startDate provided → default endDate to NOW
                startDate = this.parseFlexibleDate(args.startDate);
                ...
                if (!startDate || isNaN(startDate.getTime())) {
                    throw new Error(`Invalid startDate format: "${args.startDate}". Use "2025/10/08" or "2025-10-08"`);
                }

                endDate = new Date(); // Now
                description = `${args.startDate} to now`;

            } else if (args.endDate && !args.startDate) {
                // Only endDate provided → default startDate to 7 days before
                endDate = this.parseFlexibleDate(args.endDate);

                if (!endDate) {
                    // Fallback parsing
                    if (args.endDate.includes('T')) {
                ...
                if (!endDate || isNaN(endDate.getTime())) {
                    throw new Error(`Invalid endDate format: "${args.endDate}". Use "2025/10/08" or "2025-10-08"`);
                }
                ...
                startDate = new Date(endDate.getTime() - (7 * 24 * 60 * 60 * 1000)); // 7 days before
                description = `7 days before ${args.endDate}`;
                ...
                if (!startDate || !endDate || isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                    throw new Error(`Invalid date format. Use "2025/10/08" or "2025-10-08"`);
                }
    ...
    /**
     * Handle "download all logs" by showing available log containers and letting user choose
     * This replaces the complex parallel processing that was causing hangs
     */
    static async handleDownloadAllLogsUserChoice(args: any) {
        try {
            // Resolve project configuration
            const resolved = ProjectTools.resolveCredentials(args);
            if (!resolved.success || !resolved.credentials) {
            ...
            // Apply resolved config to args for container listing
            Object.assign(args, projectConfig);

            // If user explicitly confirmed they want to download all containers, proceed with downloads
            if (args.confirmDownloadAll === true) {
                return this.executeSimpleSequentialDownloads(args, projectConfig, projectName);
            }

            // If skipConfirmation is set, this means user wants to download but bypass the preview
            // In this case, we should proceed directly to download from available containers
            if (args.skipConfirmation === true) {
                // Get available containers first, then download from them
                const containersResult = await StorageTools.handleListStorageContainers(args);
                const containers = this.extractContainerList(containersResult);

                // Find available log containers quickly
                ...
                const availableLogContainers = standardContainers.filter(sc => containers.includes(sc));

                if (availableLogContainers.length === 0) {
                    // Fall back to pattern matching
                    const patternMatched = containers.filter(container =>
                        container.includes('log') || container.includes('console') ||
                        container.includes('application') || container.includes('http') ||
                        container.includes('web') || container.includes('insights')
                    );
                    availableLogContainers.push(...patternMatched);
                }
                ...
                // Download from found containers using the simple approach
                return this.downloadFromSpecificContainers(availableLogContainers, args, projectConfig, projectName);
            }

            // Get list of available containers
            OutputLogger.info('🔍 Listing storage containers to find log containers...');
            const containersResult = await StorageTools.handleListStorageContainers(args);
            const containers = this.extractContainerList(containersResult);

            if (containers.length === 0) {
                return ResponseBuilder.success(`## ⚠️ No Containers Found in ${args.environment}\n\n` +
                    `No storage containers were found in the ${args.environment} environment.\n\n` +
                    `**Possible causes:**\n` +
                    `• Insufficient permissions for this environment\n` +
                    `• Environment not configured for logging\n` +
                    `• API key doesn't have access to ${args.environment}\n\n` +
                    `**Try:**\n` +
                    `• Run \`check_permissions\` to verify access\n` +
                    `• Try Integration environment instead\n` +
                    `• Contact your administrator`);
            }

            // Filter containers that look like log containers - use simple, fast approach
            const logContainers = [];
            OutputLogger.info('🔍 Checking standard log containers...');

            // Use known standard containers instead of dynamic discovery to avoid hangs
            const standardContainers = [
                { name: 'insights-logs-appserviceconsolelogs', friendlyName: 'Application Logs (insights-logs-appserviceconsolelogs)' },
                { name: 'insights-logs-appservicehttplogs', friendlyName: 'Web/HTTP Logs (insights-logs-appservicehttplogs)' },
                { name: 'azure-application-logs', friendlyName: 'Application Logs (azure-application-logs)' },
                { name: 'azure-web-logs', friendlyName: 'Web/HTTP Logs (azure-web-logs)' }
            ];

            // Check which of the standard containers are actually available
            const availableContainers = standardContainers.filter(sc =>
                containers.includes(sc.name)
            );

            // If no standard containers found, fall back to pattern matching
            if (availableContainers.length === 0) {
                for (const container of containers) {
                    if (container.includes('log') || container.includes('console') ||
                        container.includes('application') || container.includes('http') ||
                        container.includes('web') || container.includes('insights')) {

                        let friendlyName = container;
                        if (container.includes('console') || container.includes('application')) {
                            friendlyName = `Application Logs (${container})`;
                        } else if (container.includes('http') || container.includes('web')) {
                            friendlyName = `Web/HTTP Logs (${container})`;
                        } else {
                            friendlyName = `Other Logs (${container})`;
                        }
                        availableContainers.push({ name: container, friendlyName });
                    }
                }
            }

            OutputLogger.info(`📋 Found ${availableContainers.length} potential log containers`);

            // Get quick preview for each available container with timeout protection
            for (const containerInfo of availableContainers) {
                const { name: container, friendlyName } = containerInfo;

                try {
                    OutputLogger.info(`📋 Checking ${container}...`);

                    // Set a strict timeout for the entire preview operation
                    const previewTimeout = new Promise((_, reject) =>
                        setTimeout(() => reject(new Error(`Preview timeout for ${container}`)), 15000) // 15 second timeout
                    );

                    const previewOperation = (async () => {
                        // Get SAS URL directly with timeout
                        const sasResponse = await StorageTools.handleGenerateStorageSasLink({
                            apiKey: projectConfig.apiKey,
                            apiSecret: projectConfig.apiSecret,
                            projectId: projectConfig.projectId,
                        ...
                        const result = await this.quickLogCheck(sasUrl, args, container);
                        return result;
                    })();

                    const result: any = await Promise.race([previewOperation, previewTimeout]);
                    ...
                    OutputLogger.info(`📭 ${container}: ${error.message}`);
                    logContainers.push({
                        container,
                        friendlyName,
                        fileCount: 0,
            ...
            if (logContainers.length === 0) {
                return ResponseBuilder.success(`## ⚠️ No Log Containers Found in ${args.environment}\n\n` +
                    `Found ${containers.length} storage containers, but none appear to contain logs.\n\n` +
                    `**Available containers:**\n` +
                    containers.map(c => `• ${c}`).join('\n') + '\n\n' +
                    `**These containers don't match typical log patterns** (log, console, application, http, web, cloudflare, insights, waf, audit)\n\n` +
                    `**Options:**\n` +
                    `• Download from a specific container: \`download logs containerName: "container-name"\`\n` +
                    `• Try a different environment: \`download logs from Integration\`\n` +
                    `• Contact Optimizely Support to enable logging\n\n` +
                    `**Note:** If you expect logs in Production, they may need to be enabled by Optimizely Support.`);
            }

            // Present the options to the user with preview information
            let message = `# 📋 Available Log Containers in ${args.environment} (with Preview)\n\n`;
            message += `Found ${logContainers.length} log container(s). Here's what's available:\n\n`;

            let totalFiles = 0;
            let containersWithFiles = 0;

            for (let i = 0; i < logContainers.length; i++) {
                const { container, friendlyName, fileCount, totalSize, hasFiles, error } = logContainers[i];

                const status = hasFiles ? '✅' : error ? '❌' : '📭';
                message += `${status} **${i + 1}. ${friendlyName}**\n`;

                if (error) {
                    message += `   ⚠️ Error: ${error}\n`;
                } else if (hasFiles) {
                    message += `   📊 **${fileCount} files** (${totalSize})\n`;
                    totalFiles += fileCount;
                    containersWithFiles++;
                } else {
                    message += `   📭 No files found for the specified time range\n`;
                }

                message += `   📥 Command: \`download logs containerName: "${container}"\`\n\n`;
            }

            // Add summary
            message += `## 📊 Summary\n`;
            message += `• **Containers with files:** ${containersWithFiles}/${logContainers.length}\n`;
            message += `• **Total files available:** ${totalFiles}\n`;
            message += `• **Time range:** ${args.daysBack ? `Last ${args.daysBack} day(s)` : 'Default (7 days)'}\n\n`;

            message += `**Download Multiple Containers:**\n`;
            message += `You can download from multiple containers by running the commands above one by one.\n`;
            message += `Each container will be saved to its own subfolder for organization.\n\n`;

            message += `**Download All at Once (Advanced):**\n`;
            if (containersWithFiles > 0) {
                message += `Download all ${containersWithFiles} container(s) with files (${totalFiles} total files):\n\n`;
                message += `**⚠️ This will download ${totalFiles} files from ${containersWithFiles} containers sequentially.**\n\n`;
                message += `To proceed with downloading all containers with files:\n`;
                message += `\`download logs logType: "all", confirmDownloadAll: true\`\n\n`;
            } else {
                message += `⚠️ No containers have files for the specified time range. Try:\n`;
                message += `• Different time range: \`download logs logType: "all", daysBack: 7\`\n`;
                message += `• Different environment: \`download logs logType: "all", environment: "Integration"\`\n\n`;
            }

            message += `**Time Range Options:**\n`;
            message += `Add time filters to any command above:\n`;
            message += `• \`... daysBack: 1\` (last 24 hours)\n`;
            message += `• \`... daysBack: 7\` (last week)\n`;
            message += `• \`... dateFilter: "2025/09/22"\` (specific date)\n`;

            return ResponseBuilder.success(message);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'download-all-logs-user-choice', args);
        }
    }

    /**
     * Execute sequential downloads of all log containers (called when user confirms)
     */
    static async executeSequentialDownloads(args: any, _projectConfig: any, _projectName: any) {
        try {
            OutputLogger.info('🚀 Starting sequential download of all log containers...');

            // Use smart container detection - try standard containers first
            const containersResult = await StorageTools.handleListStorageContainers(args);
            const containers = this.extractContainerList(containersResult);

            // Standard log containers (most common)
            const standardContainers = [
                'insights-logs-appserviceconsolelogs',
                'insights-logs-appservicehttplogs',
                'azure-application-logs',
            ...
            // Check which standard containers are available, or fall back to pattern matching
            let logContainersToCheck = standardContainers.filter(sc => containers.includes(sc));

            if (logContainersToCheck.length === 0) {
                // Fall back to pattern matching if no standard containers found
                logContainersToCheck = containers.filter(container =>
                    container.includes('log') || container.includes('console') ||
                    container.includes('application') || container.includes('http') ||
                    container.includes('web') || container.includes('insights')
                );
            ...
            OutputLogger.info(`📋 Checking ${logContainersToCheck.length} containers for downloads`);

            // Simply try to download from each container - let individual downloads handle empty containers
            const logContainersWithFiles = logContainersToCheck;

            if (logContainersWithFiles.length === 0) {
                return ResponseBuilder.success('No log containers with files found for the specified time range.');
            }

            OutputLogger.info(`📋 Found ${logContainersWithFiles.length} log containers with files to download`);

            const results = [];
            let totalDownloaded = 0;
            let totalFailed = 0;

            // Download each container sequentially (not in parallel)
            for (let i = 0; i < logContainersWithFiles.length; i++) {
                const containerName = logContainersWithFiles[i];
                OutputLogger.info(`\n🔄 [${i + 1}/${logContainersWithFiles.length}] Downloading from: ${containerName}`);

                try {
                    // Call individual download with skip confirmation and preserve incremental settings
                    if (process.env.DEBUG === 'true') {
                        console.error(`[SEQUENTIAL] Downloading ${containerName} with incremental: ${args.incremental !== false}`);
                    }

                    // Add timeout to prevent hanging on individual downloads
                    const downloadTimeout = new Promise((_, reject) =>
                        setTimeout(() => reject(new Error(`Download timeout for ${containerName}`)), 180000) // 3 minute timeout per container
                    );
                    ...
                    const downloadPromise = this.handleDownloadLogs({
                        ...
                        isPartOfBatch: true,
                        // Ensure incremental is preserved (default true unless explicitly set to false)
                        incremental: args.incremental !== false
                    });

                    const result: any = await Promise.race([downloadPromise, downloadTimeout]);

                    // Parse result to extract success/failure info including skipped files
                    if (result && result.content && result.content[0]) {
                        const text = result.content[0].text;
                        const downloadMatch = text.match(/Downloaded: (\d+) files/);
                        const failedMatch = text.match(/Failed: (\d+) files/);
                        const skippedMatch = text.match(/Skipped \(unchanged\): (\d+) files/);

                        const downloaded = downloadMatch ? parseInt(downloadMatch[1]) : 0;
                        ...
                        if (downloaded > 0 || skipped > 0) {
                            let message = `✅ ${containerName}: ${downloaded} files downloaded`;
                            if (skipped > 0) {
                                message += `, ${skipped} skipped (unchanged)`;
                            }
                            OutputLogger.success(message);
                        } else if (text.includes('No log files found') || text.includes('empty')) {
                            OutputLogger.info(`📭 ${containerName}: No logs found`);
                        } else {
                            OutputLogger.info(`⚠️ ${containerName}: ${failed} failed downloads`);
                        }

                        results.push({ containerName, downloaded, failed, skipped, success: true });
                    } else {
                        OutputLogger.error(`❌ ${containerName}: Unknown result format`);
                        results.push({ containerName, downloaded: 0, failed: 0, skipped: 0, success: false });
                    }

                } catch (error: any) {
                    OutputLogger.error(`❌ ${containerName}: ${error.message}`);
                    totalFailed++;
                    results.push({ containerName, downloaded: 0, failed: 1, success: false, error: error.message });
                }
            }
            ...
            // Generate summary
            let summary = `# 📊 Sequential Download Complete\n\n`;
            summary += `**Environment:** ${args.environment}\n`;
            summary += `**Containers processed:** ${logContainersWithFiles.length}\n`;
            summary += `**Total files downloaded:** ${totalDownloaded}\n`;
            if (totalSkipped > 0) {
                summary += `**Total files skipped (unchanged):** ${totalSkipped}\n`;
            }
            summary += `**Total failures:** ${totalFailed}\n\n`;

            if (totalSkipped > 0) {
                summary += `✨ **Smart download active**: Skipped ${totalSkipped} files that were already downloaded and unchanged.\n\n`;
            }

            summary += `**Results by container:**\n`;
            for (const result of results) {
                const status = result.success && (result.downloaded > 0 || (result.skipped || 0) > 0) ? '✅' :
                             result.downloaded === 0 && result.failed === 0 && result.skipped === 0 ? '📭' : '❌';
                summary += `${status} **${result.containerName}**: ${result.downloaded} files downloaded`;
                if ((result.skipped || 0) > 0) summary += `, ${result.skipped} skipped`;
                if (result.failed > 0) summary += ` (${result.failed} failed)`;
                if (result.error) summary += ` - ${result.error}`;
                summary += `\n`;
            }

            summary += `\n**Download completed sequentially to avoid timeouts and hangs.**\n`;

            return ResponseBuilder.success(summary);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'execute-sequential-downloads', args);
        }
    }

    /**
     * Execute direct downloads without preview checks (for skipConfirmation: true)
     * This bypasses all the complex preview logic that can cause hangs
     */
    static async executeDirectSequentialDownloads(args: any, _projectConfig: any, _projectName: any) {
        try {
            OutputLogger.info('🚀 Starting direct download of all standard log containers...');

            // Standard log containers that are commonly available
            const standardLogContainers = [
                'insights-logs-appserviceconsolelogs',  // Application logs
                'insights-logs-appservicehttplogs',     // Web/HTTP logs
                'azure-application-logs',               // Alternative application logs
                'azure-web-logs'                        // Alternative web logs
            ];
            ...
            OutputLogger.info(`📋 Attempting download from ${standardLogContainers.length} standard log containers`);

            // Download each standard container sequentially (not in parallel)
            for (let i = 0; i < standardLogContainers.length; i++) {
                const containerName = standardLogContainers[i];
                OutputLogger.info(`\n🔄 [${i + 1}/${standardLogContainers.length}] Downloading from: ${containerName}`);

                try {
                    // Call individual download directly - let it handle "no files found" gracefully
                    ...
                    if (result && result.content && result.content[0]) {
                        ...
                        if (downloaded > 0 || skipped > 0) {
                            ...
                            OutputLogger.success(message);
                            results.push({ containerName, downloaded, failed, skipped, success: true, hasFiles: true });
                        } else if (text.includes('No log files found') || text.includes('empty')) {
                            OutputLogger.info(`📭 ${containerName}: No logs found`);
                            results.push({ containerName, downloaded: 0, failed: 0, skipped: 0, success: true, hasFiles: false });
                        } else {
                            OutputLogger.info(`⚠️ ${containerName}: ${failed} failed downloads`);
                            results.push({ containerName, downloaded, failed, skipped, success: true, hasFiles: false });
                        }
                    } else {
                        OutputLogger.info(`📭 ${containerName}: Container not accessible or empty`);
                        results.push({ containerName, downloaded: 0, failed: 0, skipped: 0, success: true, hasFiles: false });
                    }

                } catch (error: any) {
                    OutputLogger.info(`📭 ${containerName}: ${error.message}`);
                    // Don't count as failures - container might just not exist in this environment
                    results.push({ containerName, downloaded: 0, failed: 0, skipped: 0, success: true, hasFiles: false, note: 'Container not available' });
                }
            }

            // Calculate total skipped files
            const totalSkipped = results.reduce((sum, r) => sum + (r.skipped || 0), 0);
            const containersWithFiles = results.filter(r => r.hasFiles).length;

            // Generate summary
            let summary = `# 📊 Direct Download Complete\n\n`;
            summary += `**Environment:** ${args.environment}\n`;
            summary += `**Containers checked:** ${standardLogContainers.length}\n`;
            summary += `**Containers with files:** ${containersWithFiles}\n`;
            summary += `**Total files downloaded:** ${totalDownloaded}\n`;
            if (totalSkipped > 0) {
                summary += `**Total files skipped (unchanged):** ${totalSkipped}\n`;
            }
            ...
            if (containersWithFiles > 0) {
                summary += `**Successful downloads:**\n`;
                for (const result of results.filter(r => r.hasFiles)) {
                    summary += `✅ **${result.containerName}**: ${result.downloaded} files downloaded`;
            ...
            const emptyContainers = results.filter(r => !r.hasFiles);
            if (emptyContainers.length > 0) {
                summary += `**Empty/unavailable containers:**\n`;
                for (const result of emptyContainers) {
                    summary += `📭 **${result.containerName}**: ${result.note || 'No files found'}\n`;
                }
                summary += `\n`;
            }

            if (totalDownloaded === 0 && containersWithFiles === 0) {
                summary += `⚠️ **No files downloaded.** This could mean:\n`;
                summary += `• No logs exist for the specified time range\n`;
                summary += `• Logging is not enabled in ${args.environment}\n`;
                summary += `• Different container names are used in your environment\n\n`;
                summary += `**Try:**\n`;
                summary += `• Different time range: \`logType: "all", daysBack: 7\`\n`;
                summary += `• Different environment: \`environment: "Integration"\`\n`;
                summary += `• List available containers: \`list_storage_containers\`\n`;
            }

            summary += `\n**Direct download completed - no preview delays or hangs.**\n`;

            return ResponseBuilder.success(summary);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'execute-direct-sequential-downloads', args);
        }
    }

    /**
     * Very simple sequential downloads - no container discovery, no preview checks
     * Just tries standard containers directly
     */
    static async executeSimpleSequentialDownloads(args: any, _projectConfig: any, _projectName: any) {
        try {
            OutputLogger.info('🚀 Starting simple download from standard log containers...');

            // Just try the most common containers directly
            ...
            for (let i = 0; i < containersToTry.length; i++) {
                const containerName = containersToTry[i];
                OutputLogger.info(`\n🔄 [${i + 1}/${containersToTry.length}] Trying: ${containerName}`);

                try {
                    // Set a 2-minute timeout for each container
                    const timeout = new Promise((_, reject) =>
                        setTimeout(() => reject(new Error(`Timeout downloading ${containerName}`)), 120000)
                    );
                    ...
                    const result: any = await Promise.race([downloadPromise, timeout]);
                    ...
                    if (result && result.content && result.content[0]) {
                        ...
                        if (downloaded > 0 || skipped > 0) {
                            ...
                            results.push({ container: containerName, downloaded, skipped, success: true });
                        } else {
                            OutputLogger.info(`📭 ${containerName}: No files found`);
                            results.push({ container: containerName, downloaded: 0, skipped: 0, success: true, empty: true });
                        }
                    } else {
                        OutputLogger.info(`📭 ${containerName}: No response or empty`);
                        results.push({ container: containerName, downloaded: 0, skipped: 0, success: false });
                    }

                } catch (error: any) {
                    OutputLogger.info(`📭 ${containerName}: ${error.message}`);
                    results.push({ container: containerName, downloaded: 0, skipped: 0, success: false, error: error.message });
                }
            }

            // Generate simple summary
            let summary = `# 📊 Simple Download Complete\n\n`;
            summary += `**Environment:** ${args.environment}\n`;
            summary += `**Containers tried:** ${containersToTry.length}\n`;
            summary += `**Containers with files:** ${successfulContainers}\n`;
            summary += `**Files downloaded:** ${totalDownloaded}\n`;
            if (totalSkipped > 0) {
                summary += `**Files skipped (unchanged):** ${totalSkipped}\n`;
                summary += `\n✨ **Smart download**: Skipped ${totalSkipped} unchanged files\n`;
            }
            summary += `\n`;

            if (successfulContainers > 0) {
                summary += `**Downloaded from:**\n`;
                results.filter(r => r.success && (r.downloaded > 0 || r.skipped > 0)).forEach(r => {
                    summary += `✅ ${r.container}: ${r.downloaded} files`;
                    if (r.skipped > 0) summary += `, ${r.skipped} skipped`;
                    summary += `\n`;
                });
            }
            ...
            if (totalDownloaded === 0 && totalSkipped === 0) {
                ...
                summary += `• No logs for the specified time range\n`;
                summary += `• Logging not enabled in ${args.environment}\n`;
                summary += `• Try different time range: \`daysBack: 7\`\n`;
            }

            summary += `\n**Simple download completed - avoided complex logic that could hang.**`;

            return ResponseBuilder.success(summary);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'execute-simple-sequential-downloads', args);
        }
    }

    /**
     * Download from specific containers (used when skipConfirmation: true)
     * This is the most direct path with minimal complexity
     */
    static async downloadFromSpecificContainers(containerNames: any, args: any, _projectConfig: any, _projectName: any) {
        try {
            OutputLogger.info(`🚀 Downloading from ${containerNames.length} log containers...`);
            ...
            for (let i = 0; i < containerNames.length; i++) {
                ...
                OutputLogger.info(`\n📥 [${i + 1}/${containerNames.length}] Downloading: ${containerName}`);

                try {
                    // Simple, direct download with timeout
                    const timeout = new Promise((_, reject) =>
                    ...
                    if (result && result.content && result.content[0]) {
                        ...
                        if (downloaded > 0 || skipped > 0) {
                            let msg = `✅ ${downloaded} files downloaded`;
                            if (skipped > 0) msg += `, ${skipped} skipped`;
                            OutputLogger.success(msg);
                            results.push({ container: containerName, downloaded, skipped, success: true });
                        } else {
                            OutputLogger.info(`📭 No files found`);
                            results.push({ container: containerName, downloaded: 0, skipped: 0, success: true, empty: true });
                        }
                    } else {
                        OutputLogger.info(`📭 No response`);
                        results.push({ container: containerName, downloaded: 0, skipped: 0, success: false });
                    }

                } catch (error: any) {
                    OutputLogger.error(`❌ ${error.message}`);
                    results.push({ container: containerName, downloaded: 0, skipped: 0, success: false, error: error.message });
                }
            }

            // Generate final summary
            const successfulContainers = results.filter(r => r.success && (r.downloaded > 0 || r.skipped > 0)).length;

            let summary = `# 📊 Download Complete\n\n`;
            summary += `**Environment:** ${args.environment}\n`;
            summary += `**Containers:** ${containerNames.length}\n`;
            summary += `**Successful:** ${successfulContainers}\n`;
            summary += `**Files downloaded:** ${totalDownloaded}\n`;
            if (totalSkipped > 0) {
            ...
            if (successfulContainers > 0) {
                summary += `\n**Results:**\n`;
                results.filter(r => r.success && (r.downloaded > 0 || r.skipped > 0)).forEach(r => {
                    summary += `✅ ${r.container}: ${r.downloaded} downloaded`;
                    if (r.skipped > 0) summary += `, ${r.skipped} skipped`;
                    summary += `\n`;
                });
            }
            ...
            if (totalDownloaded === 0 && totalSkipped === 0) {
                summary += `\n⚠️ No files downloaded. Try:\n`;
                summary += `• Different time range: \`daysBack: 7\`\n`;
                summary += `• Different environment: \`environment: "Integration"\`\n`;
            }
            ...
            return ErrorHandler.handleError(error, 'download-from-specific-containers', args);
        }
    }

    /**
     * Handle "download all logs" by automatically breaking into individual container downloads
     * This uses the proven individual download path instead of complex parallel logic
     */
    static async handleDownloadAllLogsAutomatic(args: any) {
        try {
            OutputLogger.info('🔄 Breaking "all logs" request into individual container downloads...');

            // Standard containers to try (most common)
            const standardContainers = [
                { name: 'insights-logs-appserviceconsolelogs', friendlyName: 'Application Logs' },
                { name: 'insights-logs-appservicehttplogs', friendlyName: 'Web/HTTP Logs' }
            ];
            ...
            // Show what we're doing (for both preview and download)
            let message = `# 📋 ${args.previewOnly ? 'Preview' : 'Downloading'} All Logs - Automatic Breakdown\n\n`;
            message += `**Environment:** ${args.environment}\n`;
            message += `**Time Range:** ${args.daysBack ? `Last ${args.daysBack} day(s)` : 'Default (7 days)'}\n\n`;
            message += `I'll ${args.previewOnly ? 'check' : 'download from'} these standard containers individually:\n`;
            for (const container of standardContainers) {
                message += `• **${container.friendlyName}** (${container.name})\n`;
            }
            message += `\n⏳ **Starting individual ${args.previewOnly ? 'previews' : 'downloads'}...**\n`;

            OutputLogger.info(message);

            // Download from each container individually using the proven working method
            for (let i = 0; i < standardContainers.length; i++) {
                const { name: containerName, friendlyName } = standardContainers[i];

                OutputLogger.info(`\n📥 [${i + 1}/${standardContainers.length}] ${friendlyName} (${containerName})`);

                try {
                    // Call the individual download that we know works
                    const result = await this.handleDownloadLogs({
                        ...args,
                        containerName: containerName,
                        logType: undefined, // Clear logType to avoid recursion
                        skipConfirmation: true, // Skip individual confirmations
                        isPartOfBatch: true
                    });
                    ...
                    if (result && result.content && result.content[0]) {
                        ...
                        if (downloaded > 0 || skipped > 0) {
                            successfulContainers++;
                            let msg = `✅ ${friendlyName}: ${downloaded} downloaded`;
                            if (skipped > 0) msg += `, ${skipped} skipped`;
                            OutputLogger.success(msg);
                            results.push({ container: containerName, friendlyName, downloaded, skipped, failed, success: true });
                        } else if (text.includes('No log files found') || text.includes('empty')) {
                            OutputLogger.info(`📭 ${friendlyName}: No files found`);
                            results.push({ container: containerName, friendlyName, downloaded: 0, skipped: 0, failed: 0, success: true, empty: true });
                        } else {
                            OutputLogger.info(`⚠️ ${friendlyName}: ${failed} failures`);
                            results.push({ container: containerName, friendlyName, downloaded, skipped, failed, success: true });
                        }
                    } else {
                        OutputLogger.info(`📭 ${friendlyName}: No accessible or empty`);
                        results.push({ container: containerName, friendlyName, downloaded: 0, skipped: 0, failed: 0, success: false });
                    }

                } catch (error: any) {
                    OutputLogger.error(`❌ ${friendlyName}: ${error.message}`);
                    results.push({ container: containerName, friendlyName, downloaded: 0, skipped: 0, failed: 0, success: false, error: error.message });
                }
            }

            // Generate comprehensive summary
            let summary = `# 📊 All Logs Download Complete (Automatic Breakdown)\n\n`;
            summary += `**Environment:** ${args.environment}\n`;
            summary += `**Method:** Individual container downloads\n`;
            summary += `**Containers checked:** ${standardContainers.length}\n`;
            summary += `**Successful containers:** ${successfulContainers}\n`;
            summary += `**Total files downloaded:** ${totalDownloaded}\n`;
            if (totalSkipped > 0) {
                summary += `**Total files skipped (unchanged):** ${totalSkipped}\n`;
            }
            ...
            if (totalSkipped > 0) {
                summary += `✨ **Smart download**: Skipped ${totalSkipped} files that were already downloaded and unchanged.\n\n`;
            }
            ...
            if (successfulContainers > 0) {
                ...
                for (const result of results) {
                    if (result.success && (result.downloaded > 0 || result.skipped > 0)) {
                        summary += `✅ **${result.friendlyName}**: ${result.downloaded} files downloaded`;
                        if (result.skipped > 0) summary += `, ${result.skipped} skipped`;
                        if (result.failed > 0) summary += `, ${result.failed} failed`;
                        summary += `\n`;
                    } else if (result.empty) {
                        summary += `📭 **${result.friendlyName}**: No files found for time range\n`;
                    }
                }
                summary += `\n`;
            }
            ...
            if (totalDownloaded === 0 && totalSkipped === 0) {
                ...
                summary += `• Different container names are used in your environment\n\n`;
                summary += `**Try:**\n`;
                summary += `• Different time range: \`daysBack: 7\` or \`daysBack: 30\`\n`;
                summary += `• Different environment: \`environment: "Integration"\`\n`;
                summary += `• List available containers: \`list_storage_containers\`\n\n`;
            }

            summary += `**✅ Success:** Used proven individual download method instead of complex "all" logic that was causing hangs.\n`;

            return ResponseBuilder.success(summary);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'handle-download-all-logs-automatic', args);
        }
    }

    /**
     * Guide AI clients to break "all logs" into individual container requests
     * This is more reliable than trying to automate the complex logic
     */
    static async guideClientForAllLogs(args: any) {
        try {
            let message = `# 📋 Download All Logs - Use Individual Requests\n\n`;
            message += `**Environment:** ${args.environment}\n`;
            message += `**Time Range:** ${args.daysBack ? `Last ${args.daysBack} day(s)` : 'Default (7 days)'}\n\n`;

            message += `⚠️ **For reliable downloads, please use individual container requests instead of "all":**\n\n`;

            message += `**Step 1: Download Application Logs**\n`;
            message += `\`\`\`javascript\n`;
            message += `download_logs({\n`;
            message += `    environment: "${args.environment}",\n`;
            if (args.daysBack) message += `    daysBack: ${args.daysBack},\n`;
            if (args.startDateTime) message += `    startDateTime: "${args.startDateTime}",\n`;
            if (args.endDateTime) message += `    endDateTime: "${args.endDateTime}",\n`;
            if (args.dateFilter) message += `    dateFilter: "${args.dateFilter}",\n`;
            if (args.downloadPath) message += `    downloadPath: "${args.downloadPath}",\n`;
            message += `    containerName: "insights-logs-appserviceconsolelogs",\n`;
            message += `    ${args.previewOnly ? 'previewOnly: true' : 'skipConfirmation: true'}\n`;
            message += `})\n`;
            message += `\`\`\`\n\n`;

            message += `**Step 2: Download Web/HTTP Logs**\n`;
            message += `\`\`\`javascript\n`;
            message += `download_logs({\n`;
            message += `    environment: "${args.environment}",\n`;
            if (args.daysBack) message += `    daysBack: ${args.daysBack},\n`;
            ...
            if (args.dateFilter) message += `    dateFilter: "${args.dateFilter}",\n`;
            if (args.downloadPath) message += `    downloadPath: "${args.downloadPath}",\n`;
            message += `    containerName: "insights-logs-appservicehttplogs",\n`;
            message += `    ${args.previewOnly ? 'previewOnly: true' : 'skipConfirmation: true'}\n`;
            message += `})\n`;
            message += `\`\`\`\n\n`;

            message += `**Why Individual Requests?**\n`;
            message += `• ✅ More reliable (no hanging issues)\n`;
            message += `• ✅ Better error handling per container\n`;
            message += `• ✅ Smart incremental downloads work perfectly\n`;
            message += `• ✅ Clear progress tracking\n`;
            message += `• ✅ Proven to work consistently\n\n`;

            message += `**For AI Assistants:**\n`;
            message += `Please execute the two commands above sequentially to download all logs.\n`;
            message += `This approach is much more reliable than the complex "all" logic.\n\n`;

            message += `**Alternative Containers (if standard ones don't exist):**\n`;
            message += `• Application logs: \`azure-application-logs\`\n`;
            message += `• Web logs: \`azure-web-logs\`\n\n`;

            message += `**To see all available containers:**\n`;
            message += `\`list_storage_containers(environment: "${args.environment}")\`\n`;

            return ResponseBuilder.success(message);

        } catch (error: any) {
            return ErrorHandler.handleError(error, 'guide-client-for-all-logs', args);
        }
    }

    /**
     * Quick check for log files in a container (used for previews)
     * This is a lightweight version that just counts files without full processing
     */
    static async quickLogCheck(sasUrl: any, args: any, containerName: any) {
        try {
            // Process date filters
            const dateFilter = this.processDateFilters(args);

            // Get just a very quick sample of logs to check if container has files
            // Use aggressive limit for preview speed
            const logs = await this.listLogs(sasUrl, dateFilter, containerName, 50); // Limit to first 50 for maximum speed

            const fileCount = logs.length;
            const totalSize = logs.reduce((sum: any, log: any) => sum + (log.size || 0), 0);
            ...
            if (process.env.DEBUG === 'true') {
                console.error(`[QUICK CHECK] Error checking ${containerName}: ${error.message}`);
            }
            return {
                fileCount: 0,
                totalSize: '0 B',
    ...
    /**
     * DXP-3: Handle background download - returns immediately with downloadId
     */
    static async handleBackgroundDownload(args: any, monitorProgress: any) {
        try {
            if (process.env.DEBUG === 'true') {
                console.error('[DEBUG] Background log download requested');
            ...
            // Resolve project info for download key generation
            const resolution = ProjectResolutionFix.resolveProjectSafely(args, ProjectTools);
            if (!resolution.success) {
                return ResponseBuilder.error(resolution.message || 'Failed to resolve project');
            }

            const projectName = args.projectName || (resolution.project ? resolution.project.name : 'Unknown');
            const projectConfig = resolution.project;
            const environment = args.environment || 'Production';
            const containerName = args.containerName || 'logs';

            if (process.env.DEBUG === 'true') {
                console.error('[DEBUG] Resolved project:', projectName);
            ...
            try {
                ...
                // Extract SAS URL from response
                const sasUrl = LogDownloadTools.extractSasUrl(sasResponse);

                if (sasUrl) {
                    // Process date filters
                    const dateFilter = this.processDateFilters(args);

                    // Get log files (limit to 50 for quick preview)
                    const logs = await this.listLogs(sasUrl, dateFilter, containerName, 50);

                    preview = {
                        fileCount: logs.length,
                        totalSize: logs.reduce((sum: any, log: any) => sum + (log.size || 0), 0),
                        isEstimate: logs.length >= 50  // If we hit the limit, it's an estimate
                    };

                    if (process.env.DEBUG === 'true') {
                        console.error('[DEBUG] Preview obtained:', preview.fileCount, 'files,', preview.totalSize, 'bytes');
                    }
                }
            ...
            const downloadKey = downloadManager.registerDownload({
                ...
                containerName,
                environment,
                dateRange: args.daysBack ? `past-${args.daysBack}-days` : 'all-time',
                type: 'logs',
                totalFiles: preview ? preview.fileCount : undefined,
                totalSize: preview ? preview.totalSize : undefined
            });
            ...
            this.runDownloadInBackground(downloadKey, downloadArgs).catch(error => {
                OutputLogger.error(`Background download ${downloadKey} failed:`, error.message);
                downloadManager.failDownload(downloadKey, error.message);
            });

            // Return immediately with download info
            let message = `📥 **Background Log Download Started**\n\n` +
                `Download ID: \`${downloadKey}\`\n` +
                `Project: ${projectName}\n` +
                `Environment: ${environment}\n` +
                `Container: ${containerName}\n`;
            ...
            if (preview) {
                const sizeFormatted = ManifestManager.formatBytes(preview.totalSize);
                const estimatedMinutes = Math.ceil(preview.totalSize / (1024 * 1024 * 2)); // Assume ~2MB/min

                message += `\n📊 **Download Preview**${preview.isEstimate ? ' (showing first 50 files)' : ''}:\n`;
                message += `• Files: ${preview.fileCount.toLocaleString()}${preview.isEstimate ? '+' : ''} files\n`;
                message += `• Total Size: ${sizeFormatted}${preview.isEstimate ? '+' : ''}\n`;
                message += `• Estimated Time: ~${estimatedMinutes}${preview.isEstimate ? '+' : ''} minute${estimatedMinutes !== 1 ? 's' : ''}\n`;
            }
            ...
            return ResponseBuilder.error(`Failed to start background download: ${error.message}`);
        }
    }

    /**
     * DXP-3: Run download in background (async, no await)
     */
    static async runDownloadInBackground(downloadKey: any, args: any) {
        try {
            OutputLogger.info(`📥 Starting background log download: ${downloadKey}`);

            // Call the main download handler (with background=false to prevent recursion)
            const result = await this.handleDownloadLogs(args);

            // Mark as complete in download manager
            downloadManager.completeDownload(downloadKey, result);

            OutputLogger.info(`✅ Background log download completed: ${downloadKey}`);
        } catch (error: any) {
            OutputLogger.error(`❌ Background log download failed: ${downloadKey}`, error);
...
export default LogDownloadTools;