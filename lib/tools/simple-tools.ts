...
/**
 * Environment deployments map
 */
type EnvironmentDeployments = Record<string, Deployment | null>;
...
class SimpleTools {
     ...
     */
    static async handleDeploy(args: DeployArgs): Promise<any> {
        try {
            const { target, source, project, previewOnly } = args;
            ...
            if (permissions.accessible.length === 1) {
                ...
                let response = `ℹ️ **Limited Environment Access**\n\n`;
                response += `Your API key is configured for **${env}** environment only.\n\n`;
                response += `Deployments require access to 2 environments (source and target).\n\n`;
                response += `**Available operations with single environment access:**\n`;
                ...
                response += `Contact your administrator for additional access permissions.`;

                return ResponseBuilder.success(response);
            }

            // Smart environment detection based on permissions
            let sourceEnv: string, targetEnv: string;
...
**To execute**: Run the same command without --dry-run`;
     ...
     */
    static async validateConnection(projectConfig: ProjectConfig): Promise<boolean> {
        try {
    ...
    static async getProjectConfig(projectName?: string): Promise<ProjectConfig> {
        try {
            ...
            if (projectName) {
                ...
                const project = projects.find((p: ProjectConfig) =>
                    p.name && p.name.toLowerCase() === projectName.toLowerCase()
                );

                if (!project) {
                    const availableNames = projects.map((p: ProjectConfig) => p.name).filter(Boolean).join(', ') || 'None';
                ...
                // Use default project
                const defaultProject = projects.find((p: ProjectConfig) => p.isDefault);
    ...
    static parseTargetEnvironment(target: string): string {
        if (!target) return 'Production'; // Default to production

        const targetLower = target.toLowerCase();

        // Handle common aliases - map to actual DXP environments only
    ...
    static parseSourceEnvironment(source: string): string {
        return this.parseTargetEnvironment(source);
    }

    static inferSourceEnvironment(targetEnv: string): string {
    ...
    static inferDeploymentType(sourceEnv: string, targetEnv: string): string {
    ...
    static getEnvironmentStatusIcon(deployment: Deployment | null): string {
    ...
    static getEnvironmentDetails(deployment: Deployment | null): string {
    ...
    static estimateProgress(deployment: Deployment): number {
        ...
        const typicalMinutes: Record<string, number> = {
            'Integration': 5,
            'Preproduction': 7,
            'Production': 10
        };
    ...
    static estimateETA(deployment: Deployment): string {
    ...
    static generateSuggestions(envDeployments: EnvironmentDeployments): string {
        ...
        if (envDeployments.Integration && envDeployments.Integration.status === 'Succeeded' &&
            (!envDeployments.Preproduction || new Date(envDeployments.Preproduction.startTime).getTime() < new Date(envDeployments.Integration.startTime).getTime())) {
        ...
        if (envDeployments.Preproduction && envDeployments.Preproduction.status === 'Succeeded' &&
            (!envDeployments.Production || new Date(envDeployments.Production.startTime).getTime() < new Date(envDeployments.Preproduction.startTime).getTime())) {
     ...
     */
    static async executeWithRetry<T>(operation: () => Promise<T>, operationName: string, maxRetries: number = 3): Promise<T> {
