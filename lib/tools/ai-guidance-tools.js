...
### Correct Download Pattern (Simplest → Advanced)

**⭐ EASIEST - Relative Time (recommended for most cases):**
\`\`\`javascript
// User: "Download last week's logs"
download_logs({ daysBack: 7, previewOnly: true })

// User: "Download last 6 hours"
download_logs({ hoursBack: 6, previewOnly: true })

// User: "Download last 30 minutes"
download_logs({ minutesBack: 30, previewOnly: true })
\`\`\`

**Simple - Date Strings:**
\`\`\`javascript
// User: "Download logs from October 1st onward"
download_logs({
    startDate: "2025/10/01",  // endDate defaults to NOW
    previewOnly: true
})

// User: "Download logs up to October 8th"
download_logs({
    endDate: "2025/10/08",    // startDate defaults to 7 days before
    previewOnly: true
})

// User: "Download logs from Oct 1-8"
download_logs({
...
**⚠️ ADVANCED ONLY - Precise Timestamps (use rarely):**
\`\`\`javascript
// User: "Download logs from 1am to 1:30am EST on Sept 15th"
// Step 1: Get preview
download_logs({
...
1. **USE FIRST (simplest):** \`daysBack\`, \`hoursBack\`, \`minutesBack\` - no date calculations!
2. **USE SECOND:** \`startDate\`/\`endDate\` - simple date strings like "2025/10/08"
3. **USE RARELY:** \`startDateTime\`/\`endDateTime\` - only for precise time windows

**For startDateTime/endDateTime (advanced users only):**
...
- **ALWAYS start with:** \`db_export({ previewOnly: true })\`
- **NEVER start with:** \`db_export({ autoDownload: true })\`
...
db_export({ previewOnly: false })  // WRONG! This starts export, not preview!
\\\`\\\`\\\`

**✅ CORRECT:**
\\\`\\\`\\\`javascript
// AI says: "Let me show you a preview"
db_export({ previewOnly: true })   // Correct! Actually shows preview
\\\`\\\`\\\`
...
5. AI: "Starting fresh export..."
   → \`db_export({ forceNew: true, monitor: false })\`  // Disable auto-monitoring!
6. Export starts, returns export ID
7. AI: "Checking status in 2 minutes..." (wait 2 minutes)
8. AI: Use db_export tool with ONLY exportId parameter → shows status
...
- **DO NOT CALL db_export AGAIN** - that creates a NEW export!
- Instead use db_export tool with exportId and autoDownload: true parameters
