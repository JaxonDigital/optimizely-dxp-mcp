...
import HostingDetector from '../utils/hosting-detector';
import ToolAvailabilityMatrix from '../utils/tool-availability-matrix';
import { ToolInfo } from '../utils/tool-availability-matrix';

class HostingAwareHelp {
    /**
     * Get hosting-aware help
     */
    static async handleGetHelp(args: any): Promise<any> {
        ...
        for (const [category, tools] of Object.entries(toolsByCategory)) {
            response += `**${category}**\n`;
            for (const tool of (tools as ToolInfo[])) {
        ...
        if (hostingType === HostingDetector.HOSTING_TYPES.SELF_HOSTED && unavailableTools.length > 0) {
            ...
            const unavailableByCategory: Record<string, ToolInfo[]> = {};
     ...
     * Get hosting status
     */
    static async handleGetHostingStatus(args: any): Promise<any> {
        const { FORMATTING: { STATUS_ICONS } } = Config;
...
export default HostingAwareHelp;
