...
class DeploymentFormatters {
    ...
    static formatDeploymentList(deployments, projectId = null, limit = null, projectName = null) {
        ...
        // Use passed projectId or fall back to environment variable
        ...
        if (Array.isArray(deployments) && deployments.length > 0) {
            ...
            // Apply limit if specified, otherwise show up to 10
            ...
            recentDeployments.forEach((deployment, index) => {
                ...
                // Get environment names from parameters object
                const sourceEnv = deployment.parameters?.sourceEnvironment || deployment.startEnvironment || 'Unknown';
                const targetEnv = deployment.parameters?.targetEnvironment || deployment.endEnvironment || 'Unknown';
    ...
    static formatSingleDeployment(deployment, projectName = null) {
        ...
        if (deployment.endTime) {
            const date = new Date(deployment.endTime);
            response += `• Ended: ${date.toLocaleString()}\n`;
    ...
    static formatDeploymentStarted(deployment, args) {
        const { FORMATTING: { STATUS_ICONS } } = Config;
        const projectId = args.projectId || process.env.OPTIMIZELY_PROJECT_ID;
        const projectName = args.projectName || process.env.OPTIMIZELY_PROJECT_NAME;
