...
class DeploymentFormatters {
    ...
    static formatDeploymentStarted(deployment, args) {
        ...
        response += '\n## 🎯 **Monitoring Options**:\n';
        response += `### Option 1: **Continuous Monitoring** (Recommended)\n`;
        response += `Use \`monitor_deployment\` to automatically check progress every 30 seconds:\n`;
        response += `\`\`\`\nmonitor_deployment({ deploymentId: "${deployment.id}" })\n\`\`\`\n`;

        response += `### Option 2: **Manual Status Checks**\n`;
        response += `Use \`get_deployment_status\` to check progress on demand:\n`;
        response += `\`\`\`\nget_deployment_status({ deploymentId: "${deployment.id}" })\n\`\`\`\n`;

        response += '\n**💡 Important**: Use the MCP monitoring tools above instead of bash loops.\n';
        response += 'The \`monitor_deployment\` tool provides intelligent progress tracking and automatic notifications.\n';

        if (needsVerification) {
            response += '\n**📋 Deployment Stages**:\n';
            response += '1. **In Progress** - Deployment is running\n';
            response += '2. **Verification** - Review changes at preview URL\n';
            response += '3. **Complete** - After you run `complete_deployment`\n';
        }
