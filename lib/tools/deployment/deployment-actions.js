...
class DeploymentActionOperations {
    ...
    static async handleMonitorDeployment(args) {
        ...
        try {
            ...
            const statusResult = await this.handleGetDeploymentStatus({
                deploymentId,
                apiKey, apiSecret, projectId, projectName
            });

            // Parse the initial status
            let currentStatus = 'Unknown';
            let progress = 0;
            ...
            const progressMatch = statusResult.match(/Progress:\*\*\s*(\d+)%/);
            if (progressMatch) {
            ...
            let response = [];
            response.push(`## 🔄 **Continuous Monitoring Started**\n`);
            response.push(`**Deployment ID:** ${deploymentId}`);
            response.push(`**Check Interval:** Every ${interval} seconds`);
            response.push(`**Max Duration:** ${maxDuration} minutes`);
            response.push(`**Auto-Complete:** ${autoComplete ? 'Yes' : 'No'}\n`);

            response.push(`### 📊 **Initial Status**`);
            response.push(`• **Current State:** ${currentStatus}`);
            response.push(`• **Progress:** ${progress}%\n`);

            response.push(`### 🎯 **Monitoring Behavior**`);
            response.push(`I'll check the deployment status every ${interval} seconds and notify you of:`);
            response.push(`• Progress milestones (25%, 50%, 75%, 100%)`);
            response.push(`• State transitions (InProgress → Verification → Complete)`);
            response.push(`• Any errors or issues\n`);

            if (currentStatus === 'Verification' || currentStatus === 'AwaitingVerification') {
                response.push(`### ⚠️ **Action Required**`);
                response.push(`The deployment is ready for verification!`);
                response.push(`• Review changes at the verification URLs`);
                response.push(`• Use \`complete_deployment\` to finalize`);
                response.push(`• Use \`reset_deployment\` to rollback\n`);

                if (autoComplete) {
                    response.push(`**Note:** Auto-complete is enabled but requires manual verification first.`);
                }
            }

            response.push(`### 💡 **Monitoring Tips**`);
            response.push(`• Say "stop monitoring" to cancel continuous checks`);
            response.push(`• Use \`get_deployment_status\` for immediate status`);
            response.push(`• Monitoring will stop automatically after ${maxDuration} minutes\n`);

            response.push(`---\n`);
            response.push(`**🔍 Next check in ${interval} seconds...**`);

            // Add metadata for AI to understand monitoring is active
            response.push(`\n<!-- MONITORING_ACTIVE: deploymentId=${deploymentId} interval=${interval} -->`);
