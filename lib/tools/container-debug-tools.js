    ...
    static async debugContainers(args) {
        try {
            ...
            const command = PowerShellCommandBuilder.create('Get-EpiStorageContainer')
                ...
                .addParam('ClientKey', projectConfig.apiKey)
                .addParam('ClientSecret', projectConfig.apiSecret)
                .addParam('Environment', environment)
                .build();
            
            OutputLogger.info(`📝 PowerShell Command:\n${command}\n`);
            
            // Execute with raw output using direct method
            ...
            const result2 = await PowerShellHelper.executeEpiCommandDirect(
                command,
                { 
                    parseJson: true
                }
