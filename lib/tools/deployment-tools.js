...
class DeploymentTools {
    ...
    static async startDeployment(args) {
        ...
        if (isPackageDeployment) {
            ...
            // Add flags based on what should be deployed
            // Note: When deploying code only, no flags are needed (default behavior)
            // When deploying content only or everything, we need explicit flags
