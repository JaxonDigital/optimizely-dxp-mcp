...
class SetupWizard {
    ...
    static async testConnection() {
        try {
            ...
            // Use Get-EpiStorageContainer instead of Get-EpiDeployment (which is blocked by Optimizely)
            const result = await PowerShellHelper.executeEpiCommandDirect(
                `Get-EpiStorageContainer -ProjectId '${testProject.id}' -ClientKey '${testProject.apiKey}' -ClientSecret '${testProject.apiSecret}' -Environment Integration | Select-Object -First 1`,
