            ...
            if (!forceNew) {
                ...
                if (existingBackup) {
                    ...
                    if (useExisting) {
                        ...
                        OutputLogger.info('Using existing backup as requested (--use-existing)');
                    } else if (!forceNew) {
                        ...
                        if (exportTime > oneHourAgo) {
                            shouldUseExisting = true;
                            OutputLogger.info('Auto-using recent backup (less than 1 hour old)');
                        } else {
                            ...
                            OutputLogger.warning(`📦 Found backup from ${timeAgo}`);
                            OutputLogger.info('Add --use-existing to use it, or --force-new for a fresh backup');
                            OutputLogger.info('Proceeding with new backup...\n');
                            shouldUseExisting = false;
                        }
                    }
                    
                    if (shouldUseExisting) {
                        OutputLogger.info('Using existing completed backup');
                        
                        // If auto-download requested, download the existing backup
                        if (autoDownload) {
                            const downloadDir = downloadPath || await SettingsManager.getDownloadPath();
...
${autoDownload ? `\n**Download Location**: ${downloadPath || await SettingsManager.getDownloadPath()}` : ''}
...
**To execute**: Run the same command without --dry-run`;
            ...
            let result = await DatabaseTools.handleExportDatabase({
            ...
            if (result.isError && result.content && result.content[0] && 
                ...
                OutputLogger.info('🔍 Export blocked by existing operation - checking for intelligent resolution...');
            ...
            if (result.result) {
                ...
                OutputLogger.info(`🔍 Export ID extraction result: ${exportId || 'FAILED'}`);
                if (result.result.content && result.result.content[0]) {
                    OutputLogger.info(`🔍 Response content: ${result.result.content[0].text.substring(0, 200)}...`);
                }
                ...
                if (exportId) {
                    OutputLogger.info(`✅ Export ID found: ${exportId} - Starting monitoring setup...`);
                    const downloadDir = downloadPath || await SettingsManager.getDownloadPath();
                    ...
                    OutputLogger.info(`💡 Auto-download will trigger when you check status and export is complete`);
                    ...
                    OutputLogger.info(`💡 Manual status checking will be required using backup status tool`);
                }
    ...
    static async handleExportStatus(args) {
        try {
            ...
            if (!exportId && currentState.currentExport && 
                ...
                OutputLogger.info(`Found active export for auto-download: ${targetExportId}`);
            }
                ...
                if (!targetExportId || latest) {
                    ...
                    if (targetBackup) {
                        ...
                        if (recentExport) {
                            ...
                            OutputLogger.info(`Export ${targetExportId} not found in history. Trying default environment: ${backupEnvironment}`);
                        }
                    }
                }
            }
            ...
            OutputLogger.info(`Checking backup status: ${targetExportId} (${backupEnvironment})`);
            ...
            if (result.isSuccess) {
                ...
                if (status.isComplete) {
                    ...
                    if (isCurrentExport && shouldAutoDownload && status.downloadUrl) {
                        try {
                            OutputLogger.success('🎉 Auto-download export complete! Starting download...');
...
💡 **The backup is ready, but auto-download encountered an issue**`;
                        ...
                        const capabilityCheck = await CapabilityDetector.checkAutoDownloadCapability(await SettingsManager.getDownloadPath(), 100 * 1024 * 1024);
...
${autoDownloadOffer}`;
    ...
    static async handleCheckCapabilities(args) {
        try {
            ...
            const targetPath = downloadPath || await SettingsManager.getDownloadPath();
    ...
    static async persistExportToFile(projectName, exportInfo) {
        ...
        try {
            ...
            OutputLogger.info(`📁 Export ${exportInfo.exportId} persisted to file for recovery`);
    ...
    static async recoverExportsFromFile(projectName) {
        ...
        try {
            ...
            OutputLogger.info(`💡 No export history file found for ${projectName} (this is normal for new projects)`);
            return [];
        }
    }
     ...
     */
    static async downloadFile(url, filepath, timeoutMs = 600000) { // 10 minute default timeout
        const fs = require('fs');
        const https = require('https');
        ...
        return new Promise((resolve, reject) => {
            ...
            // Overall download timeout (10 minutes default)
            const overallTimeout = setTimeout(() => {
                ...
                reject(new Error(`Download timed out after ${Math.round(timeoutMs/1000)} seconds. Try again or download manually from the DXP portal.`));
            }, timeoutMs);
            
            // Stall timeout - if no data received for 60 seconds
            let stallTimeout = setTimeout(() => {
                ...
                reject(new Error('Download stalled - no data received for 60 seconds. Network connection may be unstable.'));
            }, 60000);
            ...
            const request = https.get(url, (response) => {
                ...
                response.on('data', (chunk) => {
                    ...
                    // Reset stall timeout since we received data
                    clearTimeout(stallTimeout);
                    ...
                    if (now - lastProgressTime > 5000) {
                        ...
                        if (totalBytes) {
                            ...
                            OutputLogger.progress(`📥 Downloading: ${percent}% (${this.formatBytes(downloadedBytes)}/${this.formatBytes(totalBytes)})`);
                        } else {
                            OutputLogger.progress(`📥 Downloaded: ${this.formatBytes(downloadedBytes)}`);
                        }
                    }
                });
     ...
     */
    static async downloadFromUrl(downloadUrl, downloadPath, projectName, environment, databaseName) {
        const fs = require('fs').promises;
        const path = require('path');
    ...
    static startBackgroundMonitoring(exportId, projectConfig, environment, databaseName, downloadPath) {
        ...
        if (DatabaseSimpleTools.backgroundMonitors.has(exportId)) {
            OutputLogger.info(`🔄 Background monitoring already active for export: ${exportId}`);
        ...
        const monitorData = {
            ...
            stop: () => { 
                ...
                OutputLogger.info(`🛑 Background monitoring stopped for export: ${exportId}`);
            }
        };
        ...
        OutputLogger.info(`   ⏱️ Checking status every 5 minutes`);
        OutputLogger.info(`   📊 You'll see progress updates at: 1m, 5m, 10m, 15m, 20m, 30m, 45m`);
        OutputLogger.info(`   💾 Auto-download will trigger when export completes`);
        OutputLogger.info(`   🔍 Check manually anytime with: claude "backup status"`);
                        ...
                        if (status.isComplete && status.downloadUrl) {
                            ...
                            if (capabilityCheck.canAutoDownload) {
                                ...
                                try {
                                    ...
                                    OutputLogger.error(`❌ Auto-download failed: ${downloadError.message}`);
                                    OutputLogger.info(`📥 Manual download available: ${status.downloadUrl}`);
                                ...
                                if (clientInfo.isClaudeDesktop) {
                                    ...
                                    OutputLogger.info(`🔗 Download URL: ${status.downloadUrl}`);
                                    OutputLogger.info(`💡 Tip: Copy the URL above and open in your browser, or use Claude Code CLI for auto-download`);
                                } else {
                                    OutputLogger.success(`🎉 Export ${exportId} completed! Manual download required.`);
                                    OutputLogger.info(`📥 Manual download available: ${status.downloadUrl}`);
                                    OutputLogger.info(`⚠️ Auto-download not available: ${capabilityCheck.issues[0] || 'File system access limited'}`);
                                }
                            ...
                            if (shouldLogUpdate) {
                                ...
                                OutputLogger.info(`   📊 Status: ${status.status || 'In Progress'}`);
                                OutputLogger.info(`   🔄 Next check in 5 minutes...`);
                                
                                // Provide helpful tips at certain milestones
                                if (elapsedMinutes === 15) {
                                    OutputLogger.info(`   💡 Tip: You can check status manually anytime with: claude "backup status"`);
                                } else if (elapsedMinutes === 30) {
                                    OutputLogger.info(`   💡 Tip: Check the DXP portal directly if you're concerned about the export`);
                                } else if (elapsedMinutes === 45) {
                                    OutputLogger.info(`   💡 Tip: Consider starting a new export if this one appears stuck`);
                                }
                            }
        ...
        OutputLogger.info(`⏰ Checking every 5 minutes, will auto-download when ready`);
    ...
    static async findAvailableBackup(projectConfig, targetEnv, dbName) {
        try {
            OutputLogger.info('🔍 Checking for recent completed backups...');
            ...
            if (recentExports.length === 0) {
                OutputLogger.info('No recent backups found in history');
                return null;
            }
            ...
            if (matchingExports.length === 0) {
                OutputLogger.info('No matching recent backups found');
                return null;
            }
            ...
            OutputLogger.info(`   Export ID: ${mostRecent.exportId}`);
            OutputLogger.info(`   Environment: ${mostRecent.environment}`);
            OutputLogger.info(`   Database: ${mostRecent.databaseName}`);
    ...
    static async downloadExistingBackup(backupInfo, projectConfig, downloadPath, targetEnv, dbName) {
        ...
        try {
            ...
            if (!downloadUrl) {
                ...
                OutputLogger.info('🔍 Fetching download URL for existing backup...');
    ...
    static async resolveExportConflict(targetEnv, targetDb, projectConfig, downloadPath) {
        try {
            OutputLogger.info('🔍 Analyzing existing operations for intelligent conflict resolution...');
            ...
            for (const exportInfo of potentiallyActive) {
                try {
                    ...
                    if (statusResult.isSuccess) {
                        ...
                        if (!status.isComplete) {
                            ...
                            if (sameDatabase) {
                                ...
                                OutputLogger.info(`🔄 Switching to monitor existing export: ${exportInfo.exportId}`);
                                ...
                                this.startBackgroundMonitoring(
                                    ...
                                    downloadPath || await SettingsManager.getDownloadPath()
                                );
                                ...
                                const smartResponse = ResponseBuilder.success(
                                    ...
                                    `✅ **Auto-Download**: Will download to ${downloadPath || await SettingsManager.getDownloadPath()} when ready\n` +
                                ...
                                OutputLogger.info(`⏳ Different database export running (${exportInfo.environment} ${exportInfo.databaseName})`);
                                OutputLogger.info(`🔄 Will start ${targetEnv} ${targetDb} export when current export completes`);
                    ...
                    OutputLogger.info(`💡 Export ${exportInfo.exportId} is no longer active`);
                }
            }

            // No active exports found in our records - might be a different type of operation
            OutputLogger.info('🔍 No matching exports found - conflict might be from deployment or other operation');
    ...
    static startQueuedExportMonitoring(blockingExport, queuedExport) {
        ...
        OutputLogger.info(`🔄 Starting queued export monitoring: ${queueId}`);
    ...
    static async detectAndOfferRecovery(projectConfig) {
        try {
            ...
            if (potentiallyActiveExports.length > 0) {
                OutputLogger.info(`🔍 Found ${potentiallyActiveExports.length} potentially active export(s) that lost monitoring`);
                    ...
                    try {
                        ...
                        if (statusResult.isSuccess) {
                            ...
                            if (!status.isComplete) {
                                ...
                                const downloadPath = exportInfo.downloadPath || await SettingsManager.getDownloadPath();
                        ...
                        OutputLogger.info(`💡 Export ${exportInfo.exportId} is no longer active (this is normal)`);
                    }
                }
            }
