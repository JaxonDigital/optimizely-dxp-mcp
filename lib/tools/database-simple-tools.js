...
class DatabaseSimpleTools {
    ...
    static async handleCheckDownloadStatus(args) {
        ...
        if (download.status === 'in_progress') {
            ...
            message += `💡 **Tip**: Check again in a few minutes to see progress`;
    ...
    static async internalExportDatabase(args) {
        ...
        try {
            ...
            if (!result || result.isError || result.success === false) {
                // Extract error message from various possible locations
                let errorMsg = result?.error || result?.stderr || 'Failed to create database export';

                // Try to parse error from stderr if it contains JSON
                if (result?.stderr && result.stderr.includes('errors')) {
                    try {
                        const jsonMatch = result.stderr.match(/\{[\s\S]*"errors"[\s\S]*\}/);
                        if (jsonMatch) {
                        ...
                        // If JSON parsing fails, keep original stderr
                    }
                }
    ...
    static async internalCheckExportStatus(args) {
        ...
        try {
            ...
            return { isSuccess: true, data: result.stdout || result };
        } catch (error) {
    ...
    static async handleExportDatabase(args) {
        try {
            ...
            if (previewOnly) {
                // FIRST: Check if there's already an export in progress
                // This prevents showing the backup choice screen again if Claude Desktop re-runs the prompt
                const currentState = await this.loadCurrentExportState();
                if (currentState.currentExport) {
                    const { exportId, environment, databaseName, startTime } = currentState.currentExport;
                    const elapsed = startTime ? this.getElapsedTimeWithContext(startTime, false) : 'Unknown';

                    OutputLogger.info(`⚠️ Export already in progress: ${exportId}`);
                    ...
                    return ResponseBuilder.success(
                        ...
                        `**Started**: ${elapsed} ago\n\n` +
                        `🔍 **To check status:**\n` +
                        `\`\`\`\ncheck_export_status({ exportId: "${exportId}", environment: "${environment}" })\n\`\`\`\n\n` +
                        `💡 **Note**: Cannot start a new export while one is in progress. Check status first, or wait for completion.`
                    );
                }

                OutputLogger.debug(`[DEBUG] No export in progress. Checking for existing backups...`);
                ...
                if (!existingBackup) {
                    ...
                    preview += `**Monitoring**: ${shouldMonitor ? 'Automatic (every 2 minutes)' : 'Manual (use check_export_status)'}\n`;
                    ...
                    if (shouldMonitor) {
                        if (autoDownloadActual) {
                            ...
                            preview += `**AI: Monitor progress automatically. When complete, you will receive the file size and download URL. Show these to the user and offer to download using download_database_export tool.**\n`;
                        }
                    } else {
                        preview += `**AI: After creating export, provide the export ID. User will manually check status later using check_export_status tool.**\n`;
                    }
                    preview += `\n`;
            ...
            if (exportId) {
                ...
                if (shouldMonitor) {
                    ...
                    OutputLogger.info(`📊 To monitor: check_export_status exportId: "${exportId}" environment: "${targetEnv}"`);

                    if (autoDownloadActual) {
                        OutputLogger.info(`💾 To download when ready: download_database_export exportId: "${exportId}" environment: "${targetEnv}"`);
                    }
                }

                // Generate appropriate monitoring message based on capabilities
                const intervalDisplay = 'every 2 minutes'; // Fixed interval - simple and safe
                    ...
                    if (shouldMonitor) {
                        if (monitoringMessage.includes('Monitoring Mode')) {
                            ...
                            if (autoDownloadActual) {
                                ...
                                enhancedMessage += `• You can then download using download_database_export tool\n`;
                    ...
                    if (exportId && autoMonitor) {
                        ...
                        if (shouldMonitor) {
                            ...
                            if (autoDownloadActual) {
                                ...
                                enhancedMessage += `3. When complete, use download_database_export\n\n`;
                            }
                            ...
                            enhancedMessage += `Use \`check_export_status\` with exportId: "${exportId}", environment: "${targetEnv}", and monitor: false\n\n`;
                            enhancedMessage += `**To download when ready:**\n`;
                            enhancedMessage += `Use \`download_database_export\` with exportId: "${exportId}" and environment: "${targetEnv}"`;
                        }
                ...
                if (statusData) {
                    if (statusData.status === 'Succeeded') {
                        ...
                        if (statusData.downloadLink) {
                            ...
                            updateMessage += `download_database_export with:\n`;
            ...
            if (result.isSuccess) {
                ...
                if (status.isComplete) {
                    ...
                    if (status.downloadUrl) {
                        ...
                        // IMPORTANT: Don't download inside check_export_status - it causes MCP timeouts
                        // Instead, return immediately and instruct AI to call download_database_export
                        if (autoDownload === true) {
                            ...
                            const autoDownloadMessage = `✅ **Database Export Complete!**
...
**AI: Immediately call download_database_export to start background download:**
\`\`\`
download_database_export downloadUrl: "${status.downloadUrl}" skipConfirmation: true
\`\`\`

📥 **Note**: Download will start in background and return a download ID. Use \`check_download_status\` to monitor progress.`;
...
**AI: To download, use download_database_export with downloadUrl parameter:**
\`\`\`
download_database_export downloadUrl: "${status.downloadUrl}" skipConfirmation: true
\`\`\`
...
💡 **Export completed successfully!**`;
                    ...
                    if (shouldMonitor) {
                        statusMessage += `⏱️ **Monitoring**: Continue checking every 2 minutes\n`;
                    } else {
    ...
    static async handleDownloadDatabaseExport(args) {
        try {
            ...
            if (downloadUrl) {
                ...
                if (useBackground) {
                    ...
                    return ResponseBuilder.successWithStructuredData({
                        ...
                        `✅ Download is running in the background. Use \`check_download_status\` to monitor progress:\n` +
                        `\`\`\`\ncheck_download_status({ downloadId: "${downloadId}" })\n\`\`\`\n\n` +
                        `💡 **Tip**: Large files (200-300MB) typically take 3-5 minutes to download.\n\n` +
                        `_Download ID: ${downloadId}_`
                    );
                }
            ...
            // Get project configuration
            console.error('[download_database_export] Getting project config...');
            console.error('[download_database_export] Args keys:', Object.keys(args));
            const projectConfig = await this.getProjectConfig(
                project || projectName,
                {
                    ...args,
            ...
            console.error('[download_database_export] Got project config:', {
            ...
            if (!result.isSuccess) {
                return ResponseBuilder.error(
                    ...
                    `**Workaround**: Use download_database_export with the downloadUrl parameter instead:\n` +
                    `\`download_database_export downloadUrl: "<url>" skipConfirmation: true\`\n\n` +
            ...
            console.error('[download_database_export] Parsed status from exportId/environment path:', {
            ...
            console.error('[download_database_export] About to call downloadFromUrl with:', {
            ...
            console.error('[download_database_export] CAUGHT ERROR:', {
