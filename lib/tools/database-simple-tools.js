...
class DatabaseSimpleTools {
    ...
    static async internalCheckExportStatus(args) {
        ...
        try {
            const { apiKey, apiSecret, projectId, exportId, environment } = args;
            
            // Build command using the PowerShell command builder - Environment parameter is required
...
**To execute**: Run the same command without --dry-run`;
            ...
            if (result.result) {
                ...
                if (exportId) {
                    ...
                    const finalDownloadPath = await DownloadConfig.getDownloadPath('database', projectConfig.name, downloadPath, targetEnv);
                    
                    // Build enhanced response with monitoring info and download path
                    const enhancedMessage = originalMessage + monitoringMessage + 
                        `\n\n📂 **Download Location**: Files will be saved to \`${finalDownloadPath}\`` +
    ...
    static async downloadFromUrl(downloadUrl, downloadPath, projectName, environment, databaseName, skipConfirmation = false, incremental = true) {
        ...
        if (!skipConfirmation) {
            ...
            if (fileSize) {
                ...
                // Check for incremental download opportunity
                ...
                if (incremental) {
                    ...
                    if (manifestCheck.skippedFiles.length > 0) {
                        incrementalInfo = {
                            ...
                            existingFile: manifestCheck.skippedFiles[0],
                            savedSize: fileSize
                        };
                    }
                }
                ...
                if (incrementalInfo && incrementalInfo.alreadyDownloaded) {
                    OutputLogger.debug('\n✨ **Smart Download Detection**');
                    OutputLogger.debug(`   • This backup was already downloaded`);
                    OutputLogger.debug(`   • Existing file: ${incrementalInfo.existingFile.name}`);
                    OutputLogger.debug(`   • Bandwidth saved: ${this.formatBytes(incrementalInfo.savedSize)}`);
                    OutputLogger.debug(`   • Use --force to re-download`);
                }
                
                // Show warning for large databases
                if (sizeInGB > 1 && (!incrementalInfo || !incrementalInfo.alreadyDownloaded)) {
                ...
                if (incrementalInfo && incrementalInfo.alreadyDownloaded) {
                    return {
                        alreadyDownloaded: true,
                        filepath: path.join(downloadPath, incrementalInfo.existingFile.name),
                        fileSize: this.formatBytes(fileSize),
                        message: 'Database backup already exists locally. Skipping download.'
                    };
                }
