...
class DeploymentHelperTools {
    ...
    static async generateSasUploadUrl(args) {
        ...
        const containerResult = await PowerShellHelper.executeEpiCommand(
            containerCommand,
        ...
        const deploymentContainer = containers.find(c => 
            c.name?.includes('deployment') || 
            c.name?.includes('package')
        ) || containers[0];
        
        if (!deploymentContainer) {
            return ResponseBuilder.addFooter('No suitable deployment container found');
        }
