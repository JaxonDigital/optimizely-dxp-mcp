...
class DeploymentMonitor extends EventEmitter {
    ...
    startMonitoring(params) {
        ...
        if (this.options.debug) {
            console.error(`Started monitoring deployment ${deploymentId} (interval: ${monitorInterval}ms)`);
        }
    ...
    async _checkDeploymentStatus(monitor) {
        ...
        try {
            ...
            // Get deployment status using PowerShell
            const DeploymentActions = require('./tools/deployment/deployment-actions');
            const result = await DeploymentActions.getDeploymentStatus({
                deploymentId,
                projectId,
                apiKey,
                apiSecret
            ...
            // Parse the result to extract status and percentage
            const statusData = this._parseStatusResult(result);
