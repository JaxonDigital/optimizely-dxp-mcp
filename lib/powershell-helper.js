...
class PowerShellHelper {
    ...
    static async executeEpiCommand(command, credentials, options = {}) {
        ...
        let stdout = '';
        let stderr = '';
        let hasError = false; // DXP-72: Track if we encountered an error
        ...
        try {
            ...
            hasError = true; // DXP-72: Mark as error state
            stdout = error.stdout || '';
        ...
        // DXP-72: Fix success detection - if we caught an error or have stderr with error indicators, it's a failure
        // Check for: hasError flag, 'error' keyword, 'Exception' keyword, or ❌ emoji (used in enhanced error messages)
        const success = !hasError && (!stderr || (!stderr.includes('error') && !stderr.includes('Exception') && !stderr.includes('❌')));
