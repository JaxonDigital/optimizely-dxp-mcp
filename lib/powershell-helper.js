...
class PowerShellHelper {
    ...
    static async executeEpiCommand(command, credentials, options = {}) {
        ...
        if (!apiKey || !apiSecret || !projectId) {
            console.error('Missing credentials. Set environment variables: OPTIMIZELY_API_KEY, OPTIMIZELY_API_SECRET, OPTIMIZELY_PROJECT_ID');
            return {
                stdout: '',
                stderr: 'Missing API credentials. Please provide apiKey, apiSecret, and projectId or set environment variables.',
                parsedData: null,
                success: false
            };
        }
