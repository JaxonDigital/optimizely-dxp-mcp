...
class SelfHostedStorage {
     ...
     */
    static isSelfHostedMode(args: StorageArgs): boolean {
     ...
     */
    static buildListUrl(config: StorageConfig): string {
     ...
     */
    static buildBlobUrl(config: StorageConfig, blobName: string): string {
        const { accountName, accountKey, containerName, endpointSuffix = 'core.windows.net' } = config;
     ...
     */
    static getStorageConfig(args: StorageArgs): StorageConfig {
        ...
        const config: StorageConfig = {
            ...
            containerName: args.containerName || 'mysitemedia'
        };
     ...
     */
    static generateAuthorizationHeader(method: string, accountName: string, accountKey: string, _resourcePath: string, headers: { [key: string]: string } = {}): string {
     ...
     */
    static async listContainers(connectionString: string | ConnectionStringParts): Promise<ContainerInfo[]> {
        try {
            ...
            OutputLogger.error(`Failed to list containers: ${(error as Error).message}`);
            if (process.env.DEBUG === 'true') {
                console.error('[LIST CONTAINERS ERROR] Full error:', error);
                console.error('[LIST CONTAINERS ERROR] Stack:', (error as Error).stack);
            }
     ...
     */
    static maskConfig(config: MaskedConfig): MaskedConfig {
        const masked = { ...config };
