...
class DownloadConfig {
     ...
     * - OPTIMIZELY_VHB_DOWNLOAD_PATH: Default for VHB project
     * - OPTIMIZELY_VHB_DOWNLOAD_PATH_BLOBS: Blobs for VHB project
     * - OPTIMIZELY_CAMBRO_DOWNLOAD_PATH_BLOBS: Blobs for Cambro project
     */
    
    /**
     * Get download path for a specific type and project
     * @param {string} type - Type of download: 'blobs', 'database', 'logs'
     * @param {string} projectName - Name of the project (e.g., 'VHB', 'Cambro')
    ...
    static async getDownloadPath(type, projectName, userSpecifiedPath, environment) {
        ...
        if (projectName) {
            ...
            if (projectConfig) {
                ...
                if (compactPath) {
                    ...
                    OutputLogger.info(`📁 Using project ${type} path: ${expandedPath}`);
                    return expandedPath;
                }
            }
        }
        ...
        if (projectName) {
            ...
            if (process.env[projectTypeEnvVar]) {
                const envPath = this.expandPath(process.env[projectTypeEnvVar]);
                OutputLogger.info(`📁 Using ${projectTypeEnvVar}: ${envPath}`);
                return envPath;
            }
            ...
            if (process.env[projectEnvVar]) {
                ...
                const envPath = path.join(basePath, type.toLowerCase());
                OutputLogger.info(`📁 Using ${projectEnvVar}/${type}: ${envPath}`);
                return envPath;
            }
        }
        ...
        if (process.env[typeEnvVar]) {
            const envPath = this.expandPath(process.env[typeEnvVar]);
            OutputLogger.info(`📁 Using ${typeEnvVar}: ${envPath}`);
            return envPath;
        }
        ...
        if (process.env.OPTIMIZELY_DOWNLOAD_PATH) {
            ...
            const envPath = path.join(basePath, type.toLowerCase(), projectName || 'unknown');
            OutputLogger.info(`📁 Using OPTIMIZELY_DOWNLOAD_PATH/${type}/${projectName}: ${envPath}`);
            return envPath;
        }
        
        // Priority 6: Settings file configuration
        const settingsPath = await SettingsManager.getDownloadPath();
        if (settingsPath && settingsPath !== './backups') {
            const configPath = path.join(settingsPath, type.toLowerCase(), projectName || 'unknown');
            OutputLogger.info(`📁 Using settings path: ${configPath}`);
            return configPath;
        }
    ...
    static async getSmartDefault(type, projectName, environment) {
        ...
        const devPaths = [
            `/Users/bgerby/Documents/dev/${type}/${projectFolder}`,
            `./downloads/${type}/${projectFolder}`,
            `~/Downloads/optimizely-${type}/${projectFolder}`
        ];
        ...
        // Ultimate fallback
        const fallbackPath = path.join(process.cwd(), 'downloads', type, projectFolder);
