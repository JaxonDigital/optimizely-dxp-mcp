...
import HostingDetector from './hosting-detector';
import { HostingType, HostingDetectionArgs } from './hosting-detector';
...
/**
 * Tool information returned by queries
 */
 ...
 */
export type ToolsByCategory = Record<ToolCategory, ToolInfo[]>;
...
class ToolAvailabilityMatrix {
     ...
     */
    static TOOL_MATRIX: Record<string, ToolConfig> = {
        ...
        // Database Tools - DXP PaaS Only (DXP-81: Consolidated to 3 tools with db_ prefix)
        'db_export': {
            hostingTypes: ['dxp-paas'],
            category: 'Database',
            description: 'Start database export with auto-monitor and auto-download options',
        ...
        'db_export_download': {
            hostingTypes: ['dxp-paas'],
            category: 'Database',
            description: 'Download completed export (background or synchronous)',
            restrictedMessage: 'Database export download is only available for DXP PaaS hosting.'
        },
        ...
        'analyze_logs_streaming': {
            hostingTypes: ['dxp-paas', 'dxp-saas', 'self-hosted'],
            category: 'Storage & Downloads',
            description: 'Stream and analyze logs in-memory (2x faster, guaranteed structured output)'
        },
        'compare_logs': {
            hostingTypes: ['dxp-paas', 'dxp-saas', 'self-hosted'],
            category: 'Storage & Downloads',
            description: 'Compare baseline vs slot logs for deployment decisions'
        },
        // REMOVED: check_download_capabilities - consolidated into db_export preview mode (DXP-81)

        // Download Management - DXP-82 consolidated tools (available to all)
        'download_list': {
            hostingTypes: ['dxp-paas', 'dxp-saas', 'self-hosted'],
            category: 'Download Management',
            description: 'List and filter downloads (active, completed, failed)'
        },
        'download_status': {
            hostingTypes: ['dxp-paas', 'dxp-saas', 'self-hosted'],
            category: 'Download Management',
     ...
     */
    static isToolAvailable(toolName: string, args: HostingDetectionArgs = {}): boolean {
     ...
     */
    static getAvailableTools(args: HostingDetectionArgs = {}): ToolInfo[] {
        const currentHosting = HostingDetector.detectHostingType(args);
     ...
     */
    static getUnavailableTools(args: HostingDetectionArgs = {}): ToolInfo[] {
        const currentHosting = HostingDetector.detectHostingType(args);
        const unavailableTools: ToolInfo[] = [];
     ...
     */
    static getToolsByCategory(args: HostingDetectionArgs = {}): Partial<ToolsByCategory> {
        const availableTools = this.getAvailableTools(args);
        const grouped: Partial<ToolsByCategory> = {};
     ...
     */
    static getRestrictionMessage(toolName: string, args: HostingDetectionArgs = {}): string | null {
        const tool = this.TOOL_MATRIX[toolName];
        if (!tool) {
            return 'Unknown tool.';
        }
     ...
     */
    static getSuggestedAlternatives(restrictedToolName: string, args: HostingDetectionArgs = {}): ToolInfo[] {
...
export default ToolAvailabilityMatrix;
