...
interface Configuration {
    accounts: { [key: string]: StorageAccount };
...
interface LoadArgs {
    ...
    clientSecret?: string;
    tenantId?: string;
    logContainers?: string | string[];
    mediaContainers?: string | string[];
    useAccount?: string;
}
...
class SelfHostedConfig {
     ...
     */
    static async loadConfiguration(args: LoadArgs = {}): Promise<Configuration> {
     ...
     */
    static async loadConfigFile(): Promise<Configuration | null> {
        try {
     ...
     */
    static loadFromEnvironment(config: Configuration): void {
     ...
     */
    static loadFromArgs(config: Configuration, args: LoadArgs): void {
     ...
     */
    static parseConnectionString(connectionString: string): ConnectionStringParts {
     ...
     */
    static generateSasToken(accountName: string, accountKey: string, containerName: string, permissions: string = 'rl', expiryHours: number = 24): string {
     ...
     */
    static async discoverContainers(config: Configuration): Promise<void> {
     ...
     */
    static async saveConfiguration(config: Configuration): Promise<boolean> {
        try {
            const configDir = path.join(process.env.HOME || process.env.USERPROFILE || '', '.optimizely-mcp');
            ...
            OutputLogger.error(`Failed to save configuration: ${(error as Error).message}`);
            return false;
        }
    }
     ...
     */
    static maskSensitiveConfig(config: Configuration): Configuration {
        const masked = JSON.parse(JSON.stringify(config)); // Deep clone

        // Mask account keys
        if (masked.accounts) {
            for (const account of Object.values(masked.accounts) as any[]) {
     ...
     */
    static getAccountForContainer(config: Configuration, _containerType: string): StorageAccount {
     ...
     */
    static getContainersForType(config: Configuration, type: string): string[] {
