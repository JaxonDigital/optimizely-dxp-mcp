...
console.error('[MCP SERVER] 🚀 STARTING UP WITH OUR FIXES! 🚀', new Date().toISOString());
console.error('[MCP SERVER] ✅ DEBUG INFO AVAILABLE VIA health_check ✅');
console.error('[MCP SERVER] ✅ DATABASE PREVIEW FIXES INCLUDED ✅');
...
const schemas = {
    ...
    check_export_status: z.object({
        ...
        waitBeforeCheck: z.number().optional().describe('Wait this many seconds before checking (useful for polling, e.g., 120 for 2 minutes, 600 for 10 minutes)'),
...
const toolDefinitions = Object.keys(schemas).sort((a, b) => {
    ...
    const descriptions = {
        ...
        'export_database': '💾 Export database from any environment',
        'check_export_status': '📊 Check status of a database export (requires exportId and environment)',
        ...
        'health_check': '🔧 HIJACKED: Show process ID, session ID, uptime, and system information for debugging',
        'get_version': '🔥 TESTING - Check the current MCP server version and available updates',
        ...
        'monitor_deployment': '🔄 Continuously monitor deployment progress (AI: Use this instead of bash loops for monitoring)',
...
function setupHandlers(server) {
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (request) => {
        try {
            ...
            if (name === 'database-export-workflow') {
            ...
            throw new Error(`Unknown prompt: ${request.params?.name || 'undefined'}`);
        }
    });
    ...
    try {
        ...
        const finalText = typeof responseText === 'string'
            ? responseText
