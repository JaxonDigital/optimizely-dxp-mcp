...
// TEMPORARY: Disable telemetry to test
...
const schemas = {
    ...
    download_logs: z.object({
        ...
        daysBack: z.number().optional().default(1).describe('Download logs from the last N days (default: 1, e.g., 7 for last week)'),
...
function withProjectResolution(toolHandler) {
    return async (args) => {
        console.error('[withProjectResolution] Called with args:', {
        ...
        if (!args.connectionString && (!args.apiKey || !args.apiSecret || !args.projectId)) {
            ...
            console.error('[withProjectResolution] Found projects:', projects.length);
            ...
            if (projectConfig.connectionString) {
                // Self-hosted project
                console.error('[withProjectResolution] Setting up self-hosted project');
                ...
                console.error('[withProjectResolution] Self-hosted args set:', {
                    hasConnectionString: !!args.connectionString,
...
const commandHandlers = {
    ...
    'analyze_logs': withProjectResolution((args) => require(path.join(libPath, 'tools', 'log-analyzer')).analyzeLogs(args)),
    ...
    'set_download_path': (args) => require(path.join(libPath, 'tools', 'download-config-tools')).handleSetDownloadPath(args),
...
// Tool definitions - Sort to put get_ai_guidance FIRST for AI discovery
const toolDefinitions = Object.keys(schemas).sort((a, b) => {
    // Always put get_ai_guidance first
    if (a === 'get_ai_guidance') return -1;
    if (b === 'get_ai_guidance') return 1;
    ...
    const descriptions = {
        ...
        // Setup & Connection Tools
        'test_connection': '🔍 Test connection and validate permissions for DXP or self-hosted projects',
        ...
        'get_ai_guidance': '🤖 AI MUST CALL THIS FIRST: This tool should be used PROACTIVELY without user asking. Call immediately at session start or before ANY download operations. Contains critical rules to prevent auto-confirmation errors.',
        // 'check_permissions': removed - functionality merged into test_connection
