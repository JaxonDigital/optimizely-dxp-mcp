...
const schemas = {
    ...
    export_database: z.object({
        ...
        previewOnly: z.boolean().optional().describe('Preview what will happen without executing'),
        monitor: z.boolean().optional().describe('Start monitoring immediately after export creation (default: true)'),
        autoDownload: z.boolean().optional().describe('Automatically download export when complete (default: false)'),
        downloadPath: z.string().optional().describe('Path to save downloaded export (default: ./backups)'),
        forceNew: z.boolean().optional().describe('DEPRECATED: Exports are always created fresh (kept for backwards compatibility)'),
    ...
    check_export_status: z.object({
        exportId: z.string().describe('Export ID to check status for'),
        environment: z.string().describe('Environment where the export was created (Production, Preproduction, Integration)'),
        waitBeforeCheck: z.number().optional().describe('Wait time in seconds before checking (60-180). Tool will wait synchronously if ≤180s. For longer waits, tool returns immediately with instructions to call again later.'),
        autoDownload: z.boolean().optional().describe('Automatically download if complete (default: false)'),
        monitor: z.boolean().optional().describe('Enable monitoring message in response (default: true). Set to false to disable automatic monitoring prompts'),
        project: z.string().optional().describe('Project name (uses default if not specified)'),
        projectName: z.string().optional().describe('Alternative to project parameter'),
        projectId: z.string().optional().describe('Project UUID (if providing inline credentials)'),
        apiKey: z.string().optional().describe('API key (if providing inline credentials)'),
    ...
    download_database_export: z.object({
        exportId: z.string().optional().describe('Export ID to download (not required if downloadUrl provided)'),
        environment: z.string().optional().describe('Environment where the export was created (not required if downloadUrl provided)'),
        downloadUrl: z.string().optional().describe('Direct SAS URL to download from (skips API authentication)'),
        background: z.boolean().optional().describe('Start download in background and return immediately with download ID (default: true for files >100MB)'),
        project: z.string().optional().describe('Project name (uses default if not specified)'),
        downloadPath: z.string().optional().describe('Where to save the file (auto-detected if not specified)'),
        skipConfirmation: z.boolean().optional().describe('INTERNAL: Skip confirmation prompt (set by system after user confirms)'),
        projectName: z.string().optional().describe('Alternative to project parameter'),
        projectId: z.string().optional().describe('Project UUID (if providing inline credentials)'),
        apiKey: z.string().optional().describe('API key (if providing inline credentials)'),
        apiSecret: z.string().optional().describe('API secret (if providing inline credentials)')
    ...
    // REMOVED: check_export_status - use export_database with exportId instead
    // REMOVED: list_exports schema - tool relies on broken queryPaaSExports (DXP-49)
...
const commandHandlers = {
    ...
    'download_database_export': withProjectResolution((args) => DatabaseSimpleTools.handleDownloadDatabaseExport(args)),
    ...
    'health_check': async (args) => {
        ...
        console.error('[HEALTH_CHECK] Routing to ConnectionTestTools.healthCheck');
        const result = await ConnectionTestTools.healthCheck(args);
...
const toolDefinitions = Object.keys(schemas).sort((a, b) => {
    ...
    const descriptions = {
        ...
        'export_database': '💾 Export database from any environment (returns structured data: exportId, environment, databaseName, status, downloadUrl for automation)',
        'check_export_status': '📊 Check status of a database export (returns structured data: status, progress, downloadUrl, auto-download flag)',
        'download_database_export': '📥 Download a completed database export (returns structured data: downloadId, filePath, size, status)',
        ...
        // Log Download Tools
        'download_logs': '📊 Download logs (AI: PROACTIVELY call get_ai_guidance BEFORE using this tool - returns structured data: environment, logType, container, downloadPath, counts, size)',
        ...
        // Download Management
        'list_active_downloads': '📥 List all downloads with progress (returns structured data: totalDownloads, databaseDownloads array, logDownloads array with status/progress)',
        ...
        'get_download_status': '📊 Get detailed status of a specific download (returns structured data: downloadId, type, status, progress, metadata for both log & database downloads)',
        ...
        // Setup & Connection Tools
        'test_connection': '🔍 Test ONE connection - NOT for listing projects! Only use when explicitly asked to TEST (returns structured data: connection status, environments, capabilities)',
        'debug_info': '🔧 Show process ID, session ID, uptime, and system information for debugging',
        'health_check': '🔧 HIJACKED: Show process ID, session ID, uptime, and system information for debugging (returns structured data: status, checks object, environment access)',
        'get_version': '🔥 TESTING - Check the current MCP server version and available updates (returns structured data: currentVersion, updateAvailable, latest version, system info)',
        ...
        'list_projects': '📂 LIST ALL PROJECTS - Shows every configured project (returns structured data: totalProjects, projects array with type, credentials, paths, lastUsed)',
        ...
        'list_deployments': 'List all deployments for the configured project (returns structured data: deployments array with deploymentId, status, environments, times, progress)',
        'start_deployment': 'Start deployment between environments. Smart defaults: Upward (Int→Pre, Pre→Prod) deploys CODE; Downward (Prod→Pre/Int) copies CONTENT (returns structured data: deploymentId, status, sourceEnvironment, targetEnvironment)',
        'get_deployment_status': 'Get the status of a deployment (returns structured data: deploymentId, status, percentComplete, environments)',
        'monitor_deployment': '🔄 Continuously monitor deployment progress (AI: Use this instead of bash loops for monitoring - returns structured data with monitoring instructions)',
        'complete_deployment': 'Complete a deployment that is in Verification state (returns structured data: deploymentId, status, completedAt)',
        'reset_deployment': 'Reset/rollback a deployment (returns structured data: deploymentId, status, environments, includeDbRollback)',
        'list_storage_containers': 'List storage containers for an environment (returns structured data: environment, containerCount, containers array)',
        'generate_storage_sas_link': 'Generate SAS link for storage container (returns structured data: containerName, environment, permissions, expiresAt, sasUrl)',
        'copy_content': 'Copy content between environments (returns structured data: deploymentId, sourceEnvironment, targetEnvironment, type, status)',
    };
...
function setupHandlers(server) {
    ...
    // Final debug check
    if (toolName === 'export_database') {
        console.error('\n⚠️  [DIRECT TOOL CALL] export_database called DIRECTLY (not via prompt)');
        console.error('   If you see this INSTEAD of "[PROMPT SYSTEM]", Claude is NOT using the prompt');
    ...
    try {
        ...
        // Debug logging for export_database status checking
        if (toolName === 'export_database' && validatedArgs.exportId) {
