...
const LogAnalysisTools = require(path.join(libPath, 'tools', 'log-analysis-tools'));
...
const commandHandlers = {
    ...
    // DXP-110: Streaming log analysis (2x faster than download + analyze)
    'analyze_logs_streaming': withProjectResolution((args) => LogAnalysisTools.handleAnalyzeLogsStreaming(args)),
    // DXP-112: Compare baseline vs slot logs for deployment decisions
    'compare_logs': (args) => LogAnalysisTools.handleCompareLogs(args),
...
const toolDefinitions = Object.keys(schemas).sort((a, b) => {
    ...
    const descriptions = {
        // Simple Commands
        'deploy': '🚀 Deploy code from source to target environment - async 5-30min, auto-monitors progress (environment, [sourceEnvironment])',
        'status': '📊 Show current deployment status and environment health - returns deployment states, progress, errors ([environment], [project])',
        'quick': '⚡ Fast status check - returns only active deployments and critical issues, no detailed logs ([project])',

        // Database Export Operations
        'db_export': '💾 Start database export - async 10-60min, optional auto-monitor/download (environment, database: epicms|epicommerce, [autoMonitor], [autoDownload])',
        'db_export_status': '📊 Check database export status - returns progress %, ETA, download URL when complete (exportId, environment, [monitor], [autoDownload])',
        'db_export_download': '📥 Download completed database export to local path - background by default, returns immediately with download ID (downloadUrl, [downloadPath], [background])',

        // Storage Downloads
        'download_blobs': '📦 Download files from Azure blob storage container - async, returns file count and total size (container, environment, [downloadPath], [dateFilter])',

        // Log Operations
        'download_logs': '📊 Download application logs from environment to local path - supports date filtering and log type selection (environment, [dateFilter], [logType: http|application], [downloadPath])',
        'discover_logs': '🔎 Discover available log containers and types across all environments - returns container names, log types, and availability ([project])',
        'analyze_logs_streaming': '📊 Stream and analyze logs without downloading - faster than download+analyze (15-30sec), returns errors, performance metrics, AI agents detected (environment, [minutesBack], [logType], [slot])',
        'compare_logs': '🔍 Compare baseline vs slot logs to decide if deployment is safe - returns recommendation: proceed|investigate|abort with reasons (baseline, slot)',

        // Download Management
        'download_list': '📥 List active and completed downloads with optional filtering - returns download IDs, status, progress, file info ([status: active|completed|failed|all], [type: logs|database|all], [limit], [offset])',
        'download_status': '📊 Get real-time download status - returns progress %, speed, ETA, file path when complete (downloadId)',
        'download_cancel': '❌ Cancel specific download or all active downloads - returns cancelled count ([downloadId])',

        // Configuration
        'show_download_config': '📁 Show configured download directory path and disk space available - returns path, free space, permissions status ([project])',

        // Project Management
        'switch_project': '🔄 Switch active project context - changes default credentials for subsequent operations (projectName)',

        // System & Connection
        'test_connection': '🔍 Test connection and verify API credentials - returns environment access, permissions, project info ([project])',
        'debug_info': '🔧 Show process details, system resources, environment variables - returns PID, memory, Node version, config ()',
        'health_check': '🏥 Check MCP server health - returns telemetry status, rate limits, active monitors, system resources ()',
        'get_version': '📌 Get current MCP server version and check for updates - returns version, latest available, update available flag ()',
        'get_ai_guidance': '🤖 Get AI guidance on which tool to use for a task - returns recommended tool and parameters (taskDescription)',
        'verify_access': '🔑 Verify API credentials have access to specific environments - returns accessible environments, permission levels ([project])',
        'get_project': '📋 Get project configuration details - returns project ID, name, environments, hosting type, credentials status (projectName)',
        'update_project': '✏️ Update project configuration settings - modifies stored credentials, name, or settings (projectName, updates)',
        'list_projects': '📂 List all configured projects - returns project names, IDs, hosting types, active status ()',
        'current_project': '📌 Show currently active project context - returns project name, ID, environment access ()',
        'get_support': '💬 Get support contact information and documentation links - returns support email, docs URLs, GitHub issues link ()',

        // Monitoring
        'list_monitors': '📡 List active background deployment monitors - returns monitor IDs, deployment IDs, status, update count ()',
        'update_monitoring_interval': '⏱️ Update how often a monitor checks deployment status - changes polling interval (monitorId, intervalSeconds: 10-600)',
        'stop_monitoring': '🛑 Stop background monitoring for a deployment - stops polling, returns final status (monitorId)',
        'get_monitoring_stats': '📈 Get statistics for all monitors - returns total monitors, active count, update frequency, average duration ()',

        // System Settings
        'disable_telemetry': '🔇 Disable anonymous usage telemetry - stops collecting tool usage and error data ()',
        'enable_telemetry': '🔔 Enable anonymous usage telemetry - resumes collecting tool usage and error data for product improvement ()',
        'get_rate_limit_status': '⏳ View API rate limit status - returns requests remaining, reset time, current usage ([project])',
        'get_cache_status': '💾 View cache performance metrics - note: cache is now operation-specific, no central management ()',

        // Deployment Operations
        'list_deployments': '📋 List recent deployments - returns deployment IDs, status, source/target environments, timestamps ([project], [limit], [offset], [activeOnly])',
        'start_deployment': '🚀 Start deployment from source to target environment - async 5-30min, auto-monitors progress, returns deployment ID (sourceEnvironment, targetEnvironment, [includeBlob], [includeDB], [directDeploy])',
        'get_deployment_status': '📊 Get deployment status and progress - returns status, progress %, validation URLs if AwaitingVerification (deploymentId, [monitor], [waitBeforeCheck])',
        'monitor_deployment': '🔄 Start background monitoring for deployment - polls status every 30s, emits progress updates (deploymentId, [interval])',
        'complete_deployment': '✅ Complete deployment after verification - finalizes deployment, swaps slots (Production only), async 2-15min (deploymentId, [skipConfirmation])',
        'reset_deployment': '↩️ Rollback deployment to previous state - reverses changes, async 5-15min, optional database rollback (deploymentId, [resetWithDbRollback])',

        // Storage Operations
        'list_storage_containers': '📦 List Azure blob storage containers - returns container names, types, creation dates (environment, [project])',
        'generate_storage_sas_link': '🔗 Generate temporary access URL for storage container - returns SAS URL valid for specified hours (container, environment, [retentionHours], [writable])',

        // Content Operations
        'copy_content': '📋 Copy database and/or blob content between environments - async 30-90min, includes database and blobs (sourceEnvironment, targetEnvironment, [includeBlob], [includeDB])',
    };
